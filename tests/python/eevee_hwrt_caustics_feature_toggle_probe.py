...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Probe reflection/refraction toggle behavior with Hardware RT caustics enabled."
    )
    ...
    parser.add_argument("--reflection-mode", type=str, default="FULL")
    parser.add_argument("--refraction-mode", type=str, default="FULL")
...
def configure_scene(scene, args):
    ...
    eevee.hardware_raytracing_reflection_mode = args.reflection_mode
    eevee.hardware_raytracing_refraction_mode = args.refraction_mode
...
def ensure_world(scene):
    world = scene.world
    if world is None:
        world = bpy.data.worlds.new("HWRTCausticsFeatureToggleWorld")
        scene.world = world
...
def create_scene_layout(scene):
    floor_mat = make_diffuse_material("HWRTCausticsToggleFloor", (0.95, 0.95, 0.95, 1.0))
    mirror_mat = make_mirror_material("HWRTCausticsToggleMirror")
    glass_mat = make_glass_material("HWRTCausticsToggleGlass")
...
def run_child_render(
    output_dir: Path, samples: int, caustics_samples: int, tag: str, reflection_mode: str, refraction_mode: str
):
    command = [
        bpy.app.binary_path,
        "-b",
...
def main():
    ...
    if output_dir is None:
        cleanup_dir = tempfile.TemporaryDirectory(prefix="eevee_hwrt_caustics_feature_toggle_")
    ...
    if args.child_render:
        ...
        print(
            f"CHILD_RENDER tag={args.image_tag} reflection={args.reflection_mode} "
            f"refraction={args.refraction_mode}"
        )
        return

    failures = []

    run_child_render(output_dir, args.samples, args.caustics_samples, "full", "FULL", "FULL")
    run_child_render(output_dir, args.samples, args.caustics_samples, "reflection_off", "OFF", "FULL")
    run_child_render(output_dir, args.samples, args.caustics_samples, "refraction_off", "FULL", "OFF")
    bpy.context.view_layer.update()

    width, height, full = load_image_pixels(output_dir / "full.png")
    _width, _height, reflection_off = load_image_pixels(output_dir / "reflection_off.png")
    _width, _height, refraction_off = load_image_pixels(output_dir / "refraction_off.png")
    ...
    refraction_toggle_direct_diff = crop_max_tile_abs_diff_mean(
        full, refraction_off, width, height, direct_bounds, tiles=5
    )
    reflection_toggle_reflected_diff = crop_max_tile_abs_diff_mean(
        full, reflection_off, width, height, reflection_bounds, tiles=5
    )

    print(f"DIRECT_BOUNDS={direct_bounds}")
    print(f"REFLECTION_BOUNDS={reflection_bounds}")
    assert_metric(
        "CAUSTICS_REFRACTION_TOGGLE_DIRECT_DIFF_MEAN",
        refraction_toggle_direct_diff,
        0.002,
        failures,
    )
    assert_metric(
        "CAUSTICS_REFLECTION_TOGGLE_REFLECTED_DIFF_MEAN",
        reflection_toggle_reflected_diff,
        0.002,
        failures,
    )
    ...
    if failures:
        print("EEVEE HWRT caustics feature-toggle probe failures:")
        for failure in failures:
            print(f" - {failure}")
        sys.exit(1)

    print("EEVEE HWRT caustics feature-toggle probe passed.")
