...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Probe Hardware RT caustics toggle and sample response on a diffuse receiver."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def ensure_world(scene):
    world = scene.world
    if world is None:
        world = bpy.data.worlds.new("HWRTCausticsProbeWorld")
        scene.world = world
...
def create_scene_layout(scene):
    floor_mat = make_diffuse_material("HWRTCausticsProbeFloor", (0.95, 0.95, 0.95, 1.0))
    mirror_mat = make_mirror_material("HWRTCausticsProbeMirror")
    glass_mat = make_glass_material("HWRTCausticsProbeGlass")
    ...
    return floor, mirror, sphere, camera, target_patch
...
def main():
    ...
    if output_dir is None:
        cleanup_dir = tempfile.TemporaryDirectory(prefix="eevee_hwrt_caustics_probe_")
    ...
    ensure_world(scene)
    _floor, mirror, _sphere, camera, target_patch = create_scene_layout(scene)
    ...
    if args.child_render:
        ...
        print(
            ...
            f"samples={args.caustics_samples}"
        )
        return
    ...
    assert_metric("CAUSTICS_DIRECT_TOGGLE_DIFF_MEAN", direct_toggle_diff, 0.002, failures)
    assert_metric("CAUSTICS_DIRECT_SAMPLE_DIFF_MEAN", direct_sample_diff, 0.0005, failures)
    assert_metric("CAUSTICS_REFLECTED_TOGGLE_DIFF_MEAN", reflected_toggle_diff, 0.002, failures)
    assert_metric("CAUSTICS_REFLECTED_SAMPLE_DIFF_MEAN", reflected_sample_diff, 0.0005, failures)
    assert_upper_bound("CAUSTICS_CONTROL_TOGGLE_DIFF_MEAN", control_toggle_diff, 0.0015, failures)
    print(f"OUTPUT_DIR={output_dir}")

    if failures:
        print("EEVEE HWRT caustics probe failures:")
        for failure in failures:
            print(f" - {failure}")
        sys.exit(1)

    print("EEVEE HWRT caustics probe passed.")
