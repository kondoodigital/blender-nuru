...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Probe environment toggle behavior with Hardware RT caustics enabled."
    )
    parser.add_argument("--output-dir", type=Path, default=None)
    parser.add_argument("--samples", type=int, default=32)
    parser.add_argument("--caustics-samples", type=int, default=32)
    parser.add_argument("--child-render", action="store_true")
    parser.add_argument("--image-tag", type=str, default="")
    parser.add_argument("--environment-enabled", type=int, default=1)
...
def configure_scene(scene, args):
    ...
    eevee.use_hardware_raytracing_environment = bool(args.environment_enabled)
    eevee.use_hardware_raytracing_shadows = True
    eevee.use_hardware_raytracing_caustics = True
...
def create_environment_image(name: str, width: int = 256, height: int = 128):
    ...
    warm_dir = Vector((0.35, -0.82, 0.45)).normalized()
    cool_dir = Vector((-0.78, 0.18, 0.60)).normalized()
    ...
    for y in range(height):
        ...
        for x in range(width):
            ...
            ambient = 0.02 + 0.05 * horizon
            warm = max(direction.dot(warm_dir), 0.0) ** 14
            cool = max(direction.dot(cool_dir), 0.0) ** 18
            r = ambient + warm * 6.0 + cool * 0.15
            g = ambient * 1.2 + warm * 2.6 + cool * 0.45
            b = ambient * 1.4 + warm * 0.9 + cool * 3.8
            base = (y * width + x) * 4
            pixels[base + 0] = r
            pixels[base + 1] = g
            pixels[base + 2] = b
...
def ensure_world(scene):
    world = bpy.data.worlds.new("HWRTCausticsEnvironmentToggleWorld")
    ...
    environment.image = create_environment_image("HWRTCausticsEnvironmentToggleHDRI")
    background.inputs["Strength"].default_value = 1.0
    mapping.inputs["Rotation"].default_value[2] = math.radians(70.0)
...
def create_scene_layout(scene):
    floor_mat = make_diffuse_material("HWRTCausticsEnvToggleFloor", (0.95, 0.95, 0.95, 1.0))
    mirror_mat = make_mirror_material("HWRTCausticsEnvToggleMirror")
    glass_mat = make_glass_material("HWRTCausticsEnvToggleGlass")
    ...
    return mirror, camera, target_patch
...
def run_child_render(output_dir: Path, samples: int, caustics_samples: int, tag: str, environment_enabled: bool):
    command = [
        bpy.app.binary_path,
        "-b",
        "--factory-startup",
...
def main():
    ...
    if output_dir is None:
        cleanup_dir = tempfile.TemporaryDirectory(prefix="eevee_hwrt_caustics_environment_toggle_")
    ...
    ensure_world(scene)
    mirror, camera, target_patch = create_scene_layout(scene)
    bpy.context.view_layer.update()

    if args.child_render:
        render_to_image(scene, output_dir, args.image_tag)
        print(f"CHILD_RENDER tag={args.image_tag} environment={bool(args.environment_enabled)}")
        return

    failures = []

    run_child_render(output_dir, args.samples, args.caustics_samples, "environment_on", True)
    run_child_render(output_dir, args.samples, args.caustics_samples, "environment_off", False)
    bpy.context.view_layer.update()

    width, height, environment_on = load_image_pixels(output_dir / "environment_on.png")
    _width, _height, environment_off = load_image_pixels(output_dir / "environment_off.png")
    ...
    direct_diff = crop_max_tile_abs_diff_mean(environment_on, environment_off, width, height, direct_bounds, tiles=5)
    reflected_diff = crop_max_tile_abs_diff_mean(
        environment_on, environment_off, width, height, reflection_bounds, tiles=5
    )

    print(f"DIRECT_BOUNDS={direct_bounds}")
    print(f"REFLECTION_BOUNDS={reflection_bounds}")
    assert_metric("CAUSTICS_ENVIRONMENT_TOGGLE_DIRECT_DIFF_MEAN", direct_diff, 0.0010, failures)
    assert_metric("CAUSTICS_ENVIRONMENT_TOGGLE_REFLECTED_DIFF_MEAN", reflected_diff, 0.0010, failures)
    print(f"OUTPUT_DIR={output_dir}")

    if failures:
        print("EEVEE HWRT caustics environment-toggle probe failures:")
        for failure in failures:
            print(f" - {failure}")
        sys.exit(1)

    print("EEVEE HWRT caustics environment-toggle probe passed.")
