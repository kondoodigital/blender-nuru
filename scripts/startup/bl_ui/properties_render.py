...
_NURU_LOGO_PREVIEW = None
_NURU_LOGO_ICON_NAME = "nuru_logo"


def _nuru_logo_filepath():
    datafiles_root = bpy.utils.system_resource('DATAFILES')
    candidates = [
        os.path.normpath(
            os.path.join(os.path.dirname(__file__), "..", "..", "..", "images", "nuru_logo_512.png")
        ),
        os.path.join(datafiles_root, "icons", "nuru_logo_512.png") if datafiles_root else "",
    ]
...
def _nuru_logo_icon_id():
    global _NURU_LOGO_PREVIEW

    filepath = _nuru_logo_filepath()
    if not os.path.exists(filepath):
        return 0

    if _NURU_LOGO_PREVIEW is None:
        import bpy.utils.previews
        pcoll = bpy.utils.previews.new()
        pcoll.load(_NURU_LOGO_ICON_NAME, filepath, 'IMAGE')
        _NURU_LOGO_PREVIEW = pcoll

    preview = _NURU_LOGO_PREVIEW.get(_NURU_LOGO_ICON_NAME)
...
class RENDER_PT_eevee_raytracing(RenderButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        if props.ray_tracing_method == 'HARDWARE':
            supports_hardware_rt = props.is_hardware_raytracing_supported
            status_text = props.hardware_raytracing_support_label
            status_icon = 'INFO' if supports_hardware_rt else 'ERROR'
            logo_icon_id = _nuru_logo_icon_id()
            status_box = layout.box()
            status_box.use_property_split = False
            status_box.use_property_decorate = False
            if logo_icon_id:
                logo_row = status_box.row()
                logo_row.alignment = 'CENTER'
                logo_row.template_icon(icon_value=logo_icon_id, scale=8.0)
            ...
            quick_box.use_property_split = False
            quick_box.use_property_decorate = False
            quick_col = quick_box.column(align=True)
            quick_col.active = supports_hardware_rt
            quick_col.label(text="Quick Settings")

            sub = quick_col.row(align=True)
            sub.use_property_split = True
            sub.use_property_decorate = False
            sub.active = supports_hardware_rt
            sub.prop(options, "resolution_scale", expand=True)

            section = quick_col.column(align=True)
            row = section.row(align=True)
            row.prop(props, "use_hardware_raytracing_gi", text="Global Illumination")
            sub = section.row(align=True)
            sub.active = props.use_hardware_raytracing_gi
            sub.prop(options, "gi_spatial_samples", text="GI Spatial")
            section = quick_col.column(align=True)
            section.label(text="Raytrace Reflections")
            row = section.row(align=True)
            row.prop_enum(props, "hardware_raytracing_reflection_mode", 'OFF', text="Off")
            row.prop_enum(props, "hardware_raytracing_reflection_mode", 'FULL', text="On")
            sub = section.row(align=True)
            sub.active = props.hardware_raytracing_reflection_mode != 'OFF'
            sub.prop(props, "ray_tracing_reflection_bounces", text="Bounces")

            section = quick_col.column(align=True)
            section.label(text="Raytrace Refractions")
            row = section.row(align=True)
            row.prop_enum(props, "hardware_raytracing_refraction_mode", 'OFF', text="Off")
            row.prop_enum(props, "hardware_raytracing_refraction_mode", 'FULL', text="On")
            sub = section.row(align=True)
            sub.active = props.hardware_raytracing_refraction_mode != 'OFF'
            sub.prop(props, "ray_tracing_refraction_bounces", text="Bounces")

            section = quick_col.column(align=True)
            section.label(text="Raytrace Shadows")
            row = section.row(align=True)
            row.prop(props, "use_hardware_raytracing_shadows", text="On")
            sub = section.row(align=True)
            sub.active = props.use_hardware_raytracing_shadows
            sub.prop(props, "hardware_raytracing_shadow_samples", text="Samples")
            sub = section.row(align=True)
            sub.active = props.use_hardware_raytracing_shadows
            sub.prop(props, "hardware_raytracing_shadow_transparency", text="Transparent Shadows", slider=True)
            sub = section.row(align=True)
            sub.active = props.use_hardware_raytracing_shadows
            sub.prop(props, "hardware_raytracing_shadow_color_intensity", text="Color Transmission", slider=True)

            section = quick_col.column(align=True)
            section.label(text="Indirect Light")
            row = section.row(align=True)
            row.prop(props, "indirect_light_intensity", text="Strength")

            section = quick_col.column(align=True)
            section.label(text="Indirect Clamp")
            row = section.row(align=True)
            row.prop(props, "clamp_surface_indirect", text="Value")
            ...
            sub = section.row(align=True)
            sub.prop(options, "denoise_quality", text="Quality")
            sub = section.row(align=True)
            sub.prop(options, "denoise_sample_interval", text="Denoise Sample Interval")
            sub = section.row(align=True)
            sub.prop(options, "denoise_use_gpu", text="Use GPU")
            sub = section.row(align=True)
            sub.prop(options, "denoise_spatial")
            sub = section.row(align=True)
            sub.active = options.denoise_spatial
            sub.prop(options, "denoise_temporal")
...
class RENDER_PT_eevee_gi_approximation(RenderButtonsPanel, Panel):
    ...
    def poll(cls, context):
        return (context.engine in cls.COMPAT_ENGINES and
                context.scene.eevee.ray_tracing_method != 'HARDWARE')
    ...
    def draw(self, context):
        ...
        is_hardware_method = props.ray_tracing_method == 'HARDWARE'

        col = layout.column()
        col.active = props.use_raytracing and props.use_fast_gi and not is_hardware_method
        col.prop(options, "trace_max_roughness", text="Threshold")

        is_valid = (
            props.use_raytracing and
            props.use_fast_gi and
            not is_hardware_method and
            props.ray_tracing_options.trace_max_roughness < 1
        )
        ...
        if is_hardware_method:
            info = layout.column(align=True)
            info.label(text="Fast GI is disabled for the", icon='INFO')
            info.label(text="experimental hardware method.")
...
class RENDER_PT_eevee_denoise(RenderButtonsPanel, Panel):
    ...
    def draw(self, context):
        scene = context.scene
        scene_props = scene.eevee

        layout = self.layout
        layout.active = scene_props.use_raytracing
        layout.use_property_split = True
        layout.use_property_decorate = False
        props = context.scene.eevee.ray_tracing_options

        col = layout.column(heading="Stages")
...
def unregister():
    global _NURU_LOGO_PREVIEW
    if _NURU_LOGO_PREVIEW is not None:
        import bpy.utils.previews
        bpy.utils.previews.remove(_NURU_LOGO_PREVIEW)
        _NURU_LOGO_PREVIEW = None
