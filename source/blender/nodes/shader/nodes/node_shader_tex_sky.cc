...
static void node_shader_buts_tex_sky(ui::Layout &layout, bContext *C, PointerRNA *ptr)
{
  ...
  if (RNA_enum_get(ptr, "sky_type") == SHD_SKY_PREETHAM) {
    ...
    const bool use_nuru_raytracing = (scene != nullptr) &&
                                     ((scene->eevee.flag & SCE_EEVEE_SSR_ENABLED) != 0);
    if (scene != nullptr && BKE_scene_uses_blender_eevee(scene) && !use_nuru_raytracing) {
      layout.label(RPT_("Sun Disk works only on Nuru Raytracing"), ICON_ERROR);
    }
                                   ...
                                   GPUNodeStack *out)
{
  ...
  return GPU_stack_link(mat,
                        ...
                        GPU_constant(&sun_elevation),
                        GPU_constant(&tex->sun_size),
                        GPU_constant(&tex->sun_intensity),
                        GPU_constant(&sun_disc),
                        GPU_uniform(xyz_to_rgb.r),
                        GPU_uniform(xyz_to_rgb.g),
                        GPU_uniform(xyz_to_rgb.b),
                        sky_texture,
