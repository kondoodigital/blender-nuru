...
void MTLBackend::capabilities_init(MTLContext *ctx)
{
  ...
  GCaps.hardware_raytracing_support = false;
  MTLBackend::capabilities.supports_texture_atomics = false;
#if defined(MAC_OS_VERSION_14_0)
  if (@available(macOS 14.0, *)) {
    const bool is_pre_m3_apple_gpu = MTLBackend::capabilities.gpu == APPLE_GPU_M1 ||
                                     MTLBackend::capabilities.gpu == APPLE_GPU_M2;
    MTLBackend::capabilities.supports_texture_atomics = true;
    GCaps.hardware_raytracing_support = [device supportsRaytracing] && !is_pre_m3_apple_gpu;
  }
#endif
  ...
  GCaps.max_textures_geom = GCaps.max_textures;
