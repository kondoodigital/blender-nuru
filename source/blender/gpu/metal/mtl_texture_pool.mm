...
static bool texture_pool_assert_abort_enabled()
{
  const char *value = std::getenv("BLENDER_GPU_METAL_IGNORE_TEXTURE_POOL_ASSERT");
  const bool ignore_assert = (value != nullptr) && (value[0] != '\0') &&
                             !(value[0] == '0' && value[1] == '\0');
  return !ignore_assert;
}
...
void MTLTexturePool::reset(bool force_free)
{
  ...
  const bool assert_on_missing_release = texture_pool_assert_abort_enabled();
  for (const TextureHandle &tex : acquired_) {
    if (tex.users_count == 0) {
      continue;
    }
    if (assert_on_missing_release) {
      ...
      CLOG_WARN(&LOG,
                "Ignoring Metal texture-pool users_count=%d during reset because "
                "BLENDER_GPU_METAL_IGNORE_TEXTURE_POOL_ASSERT is set.",
                tex.users_count);
    }
  }
#endif
