    ...
    MTLBatch::VertexDescriptorShaderInterfacePair &data)
{
  ...
  if (!reported_cache_full_) {
    printf("[Performance Warning] cache is full (Size: %d), vertex descriptor will not be cached\n",
           GPU_VAO_STATIC_LEN);
    reported_cache_full_ = true;
  }
  return false;
}
