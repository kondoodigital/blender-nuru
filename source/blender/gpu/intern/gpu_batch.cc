                                       ...
                                       VertBuf *vbo)
{
  ...
  for (uint a_idx = 0; a_idx < format->attr_len; a_idx++) {
    ...
    for (uint n_idx = 0; n_idx < a->name_len; n_idx++) {
      ...
      if ((input == nullptr) && STREQ(name, "a")) {
        input = interface->ssbo_get("eevee_default_uv_attr");
      }
      if (input == nullptr || input->location == -1) {
        continue;
      }
      if (bound_attr & (1 << input->location)) {
        /* Keep the first resolved source for an SSBO attribute slot. This avoids later aliases
         * silently overriding default bindings such as the active render UV alias `"a"`. */
        continue;
      }
      ...
      char uniform_name_meta[] = "gpu_attr_0_meta";
      uniform_name_meta[9] = '0' + input->location;
      const int meta_loc = GPU_shader_get_uniform(shader, uniform_name_meta);
      if (meta_loc != -1) {
        const int meta = int(a->type.comp_len()) | (int(a->type.comp_type()) << 8) |
                         (int(a->type.fetch_mode()) << 16);
        GPU_shader_uniform_int_ex(shader, meta_loc, 1, 1, &meta);
      }
