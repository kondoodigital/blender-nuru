...
bool GPU_hardware_raytracing_support()
{
  return GCaps.hardware_raytracing_support;
}

bool GPU_viewport_hardware_raytracing_support()
{
  /* Keep the Eevee viewport HWRT contract explicit instead of inferring it from the raw device
   * capability bit alone. Metal (Apple Silicon) and Vulkan (NVIDIA RTX) own real Eevee viewport
   * HWRT paths; other backends can extend this gate once they do too. The per-device gating
   * lives in the backend capability bit checked above. */
