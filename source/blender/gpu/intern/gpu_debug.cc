...
/* Nuru: keep debug groups flowing when the Vulkan checkpoint wedge diagnostic is active
 * (BLENDER_VULKAN_CHECKPOINTS=1) so checkpoint markers can name the originating pass without
 * enabling --debug-gpu (which would add validation layers and perturb the failing cadence).
 * Kept backend-agnostic on purpose: a plain env check, no Vulkan includes. */
static bool gpu_debug_groups_forced_by_diagnostic()
{
...
{
  if (!(G.debug & G_DEBUG_GPU) && !G.profile_gpu && !gpu_debug_groups_forced_by_diagnostic()) {
    return;
  }
