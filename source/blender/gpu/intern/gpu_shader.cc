                     ...
                     const shader::SpecializationConstants *constants_state)
{
  ...
  const shader::SpecializationConstants *resolved_constants = constants_state;
  if (resolved_constants == nullptr && !shader->constants->is_empty()) {
    resolved_constants = shader->constants.get();
  }

  if (ctx->shader != shader) {
    ctx->shader = shader;
    ctx->shader_constants = resolved_constants ? *resolved_constants : shader::SpecializationConstants{};
    shader->bind(resolved_constants);
    ...
    if (resolved_constants) {
      ctx->shader_constants = *resolved_constants;
      shader->bind(resolved_constants);
    }
    else {
      ctx->shader_constants = shader::SpecializationConstants{};
    }
