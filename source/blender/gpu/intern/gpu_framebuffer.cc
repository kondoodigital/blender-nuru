                           ...
                           uint clear_stencil)
{
  ...
  const double4 clear_col_double = (clear_col != nullptr) ?
                                       double4(clear_col[0],
                                               clear_col[1],
                                               clear_col[2],
                                               clear_col[3]) :
  ...
  if ((G.debug & G_DEBUG_GPU) && (buffers & GPU_COLOR_BIT)) {
    for (int i = 0; i < GPU_FB_MAX_COLOR_ATTACHMENT; i++) {
      ...
      if (tex != nullptr) {
        check_clear_color(tex, clear_col_double, i);
      }
    }
  }

  gpu_fb->clear(buffers, clear_col_double, clear_depth, clear_stencil);
}
...
void GPU_framebuffer_multi_clear(gpu::FrameBuffer *fb, const float (*clear_colors)[4])
{
  ...
  double4 clear_colors_double[GPU_FB_MAX_COLOR_ATTACHMENT];
  for (int i = 0; i < GPU_FB_MAX_COLOR_ATTACHMENT; i++) {
    /* Only read entries backed by an attachment. The caller's array is only required to cover
     * attached slots, so reading all GPU_FB_MAX_COLOR_ATTACHMENT entries overflows it. */
    if (fb->color_tex(i) != nullptr) {
      clear_colors_double[i] = double4(
          clear_colors[i][0], clear_colors[i][1], clear_colors[i][2], clear_colors[i][3]);
    }
    else {
      clear_colors_double[i] = double4(0.0);
    }
  }
  ...
  if (G.debug & G_DEBUG_GPU) {
    for (int i = 0; i < GPU_FB_MAX_COLOR_ATTACHMENT; i++) {
      ...
      if (tex != nullptr) {
        check_clear_color(tex, clear_colors_double[i], i);
      }
    }
  }

  fb->clear_multi(Span<double4>(clear_colors_double, GPU_FB_MAX_COLOR_ATTACHMENT));
}
                                   ...
                                   char err_out[256])
{
  ...
  if (clear) {
    ...
    if (with_depth_buffer) {
      GPU_framebuffer_clear_color_depth(fb, clear_color, 0.0f);
    }
    else {
