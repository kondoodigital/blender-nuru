...
float3 nishita_sun_direction(float sun_elevation, float sun_rotation)
{
  float longitude = sun_rotation - M_PI_2;
  return float3(cos(sun_elevation) * cos(longitude),
                cos(sun_elevation) * sin(longitude),
                sin(sun_elevation));
}
                          ...
                          float4 &color)
{
  ...
  if (sun_disc > 0.5f && sun_size > 1.0e-6f) {
    float3 direction = normalize(co);
    float3 sun_direction = nishita_sun_direction(sun_elevation, sun_rotation);
    float sun_gamma = acos(clamp(dot(direction, sun_direction), -1.0f, 1.0f));
    float sun_disk = 1.0f - smoothstep(sun_size * 0.85f, sun_size, sun_gamma);
    color.rgb += float3(sun_disk * max(sun_intensity, 0.0f));
  }
}
