                          ...
                          float4 &color)
{
  ...
  if (sun_disc > 0.5f && sun_size > 0.0f && sun_intensity > 0.0f) {
    float gamma = sky_angle_between(
        spherical.x, spherical.y, M_PI_2 - sun_elevation, -sun_rotation);
    float sun_mask = smoothstep(sun_size, sun_size * 0.8f, gamma);
    color.rgb += float3(sun_mask * sun_intensity);
  }
}
