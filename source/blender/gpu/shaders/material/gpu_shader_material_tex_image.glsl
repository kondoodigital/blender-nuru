...
{
#ifdef MAT_HIT_EVAL
  /* Sparse HWRT hit-eval replays materials on tiny proxy triangles. Neighboring proxy pixels can
   * belong to unrelated traced hits, so hardware derivatives are not a valid texture footprint.
   * Use an explicit base LOD to keep reflected texture tiling stable in mirrors. */
  color = textureLod(ima, co.xy, 0.0f);
#else
...
/* Sphere/tube projection remap UVs with a derivative discontinuity. Do not apply the roughness
 * material-filter LOD inflation here; it blows up textureGrad at poles and seams. */
[[node]]
void node_tex_image_linear_projected(float3 co, sampler2D ima, float4 &color, float &alpha)
{
#ifdef MAT_HIT_EVAL
  color = textureLod(ima, co.xy, 0.0f);
#else
#ifdef GPU_FRAGMENT_SHADER
  float2 dx = gpu_dfdx(co.xy) * texture_lod_bias_base_get();
  float2 dy = gpu_dfdy(co.xy) * texture_lod_bias_base_get();
...
[[node]]
void node_tex_image_cubic_projected(float3 co, sampler2D ima, float4 &color, float &alpha)
{
#ifdef MAT_HIT_EVAL
  color = textureLod(ima, co.xy, 0.0f);
  alpha = color.a;
    ...
    float3 texco, float3 N, sampler2D ima, float4 &color1, float4 &color2, float4 &color3)
{
  ...
  float alpha;
  node_tex_image_linear(uv.xyy, ima, color1, alpha);
  /* Y projection */
  uv = texco.xz;
  if (N.y > 0.0f) {
    uv.x = 1.0f - uv.x;
  }
  node_tex_image_linear(uv.xyy, ima, color2, alpha);
  /* Z projection */
  uv = texco.yx;
  if (N.z > 0.0f) {
    uv.x = 1.0f - uv.x;
  }
  node_tex_image_linear(uv.xyy, ima, color3, alpha);
}
    ...
    float3 co, sampler2DArray ima, sampler1DArray map, float4 &color, float &alpha)
{
  if (node_tex_tile_lookup(co, ima, map)) {
    ...
    float2 dx = gpu_dfdx(co.xy) * texture_lod_bias_get();
    float2 dy = gpu_dfdy(co.xy) * texture_lod_bias_get();
    color = textureGrad(ima, co, dx, dy);
#else
