                          ...
                          Closure &result)
{
  ...
  float3 metallic_layer_color = float3(0.0f);
  if (metallic > 0.0f) {
    ...
    metallic_layer_color = weight * metallic * clamped_base_color.rgb;
    /* Attenuate lower layers */
  ...
  if (true) {
#ifdef MAT_HIT_EVAL
    /* Mirrored HWRT replay exports metal + diffuse only. Skip the dielectric specular LUT entirely
     * so raising Principled IOR cannot crush the replayed diffuse layer through Fresnel weighting.
     */
    ClosureReflection reflection_data;
    reflection_data.N = N;
    reflection_data.roughness = roughness;
    reflection_data.color = metallic_layer_color * coat_tint.rgb;
