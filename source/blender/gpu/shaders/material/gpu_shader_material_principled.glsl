...
float principled_opaque_specular_fade(float roughness)
{
  /* Let fully rough opaque Principled materials resolve as diffuse instead of a very broad
   * metal/specular lobe. Transmission is handled separately and must not use this fade. */
  return 1.0f - smoothstep(0.5f, 0.999f, roughness);
}
                          ...
                          Closure &result)
{
  ...
  /* Metallic component */
  const float opaque_specular_fade = principled_opaque_specular_fade(roughness);
  float3 reflection_tint = specular_tint.rgb;
  float3 reflection_color = float3(0.0f);
  float faded_metallic = metallic * opaque_specular_fade;
  if (faded_metallic > 0.0f) {
    ...
    reflection_color = weight * faded_metallic * metallic_brdf;
    /* Attenuate lower layers */
    weight *= max((1.0f - faded_metallic), 0.0f);
  }
  ...
  if (true) {
    ...
    reflectance *= opaque_specular_fade;
