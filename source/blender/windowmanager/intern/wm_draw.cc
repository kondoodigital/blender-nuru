...
static constexpr double WM_NURU_UI_VIEW3D_REDRAW_INTERVAL = 1.0 / 12.0;

static bool wm_window_has_active_non_view3d_ui_interaction(const wmWindow *win)
{
  return win->runtime != nullptr && win->runtime->non_view3d_ui_interaction_count > 0;
}

static bool wm_area_uses_nuru_viewport(const Scene *scene, const ScrArea *area)
{
  if (scene == nullptr || area->spacetype != SPACE_VIEW3D ||
      !STREQ(scene->r.engine, RE_engine_id_BLENDER_EEVEE))
  {
    return false;
  }

  const View3D *v3d = static_cast<const View3D *>(area->spacedata.first);
  return v3d != nullptr && v3d->shading.type == OB_RENDER &&
         ((scene->eevee.flag & SCE_EEVEE_SSR_ENABLED) != 0) &&
         scene->eevee.ray_tracing_method == RAYTRACE_EEVEE_METHOD_HARDWARE;
}

static bool wm_region_should_throttle_nuru_view3d_redraw(const Scene *scene,
                                                         const wmWindow *win,
                                                         const ScrArea *area,
                                                         const ARegion *region,
                                                         const double time_now)
{
  if (!wm_window_has_active_non_view3d_ui_interaction(win) || !wm_area_uses_nuru_viewport(scene, area)) {
    return false;
  }
  if (region->regiontype != RGN_TYPE_WINDOW || region->runtime->do_draw == 0) {
    return false;
  }
  return time_now < win->runtime->next_nuru_view3d_redraw_time;
}

static void wm_window_note_nuru_view3d_redraw(wmWindow *win,
                                              const Scene *scene,
                                              const ScrArea *area,
                                              const ARegion *region,
                                              const double time_now)
{
  ...
  {
    win->runtime->next_nuru_view3d_redraw_time = time_now + WM_NURU_UI_VIEW3D_REDRAW_INTERVAL;
  }
}
...
static void wm_draw_area_offscreen(bContext *C, wmWindow *win, ScrArea *area, bool stereo)
{
  ...
  const double time_now = wm_window_has_active_non_view3d_ui_interaction(win) ?
  ...
  for (ARegion &region : area->regionbase) {
    ...
    if (wm_region_should_throttle_nuru_view3d_redraw(scene, win, area, &region, time_now)) {
      continue;
    }
    ...
    wm_window_note_nuru_view3d_redraw(win, scene, area, &region, BLI_time_now_seconds());
