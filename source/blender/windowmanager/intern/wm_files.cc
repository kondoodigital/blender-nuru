  ...
  if (U.file_preview_type != USER_FILE_PREVIEW_NONE) {
    ...
    if (thumb != nullptr) {
      ...
      if (file_preview_type == USER_FILE_PREVIEW_AUTO) {
        ...
        bScreen *screen = CTX_wm_screen(C);
        ScrArea *view3d_area = (screen != nullptr) ?
                                   BKE_screen_find_big_area(screen, SPACE_VIEW3D, 0) :
                                   nullptr;
        bool do_render = (scene != nullptr && scene->camera != nullptr && view3d_area != nullptr);
        if (do_render) {
          const View3D *v3d = static_cast<const View3D *>(view3d_area->spacedata.first);
          if (v3d != nullptr && v3d->shading.type == OB_RENDER) {
            /* Saving from a rendered viewport must not synchronously launch another Eevee/HWRT
             * offscreen render just to create the blend thumbnail. The current front-buffer is
             * already representative and avoids making simple scene saves wait on viewport render
             * work. */
            do_render = false;
          }
        }
