...
struct HandleButtonData {
  ...
  bool throttles_nuru_view3d_redraw = false;
...
static bool button_state_throttles_nuru_view3d_redraw(const HandleButtonState state)
{
  return ELEM(state,
              BUTTON_STATE_WAIT_RELEASE,
              BUTTON_STATE_NUM_EDITING,
...
static bool button_interaction_is_non_view3d_ui(const HandleButtonData *data)
{
  if (data->window == nullptr || data->window->runtime == nullptr || data->region == nullptr) {
    return false;
  }
  return !(data->area != nullptr && data->area->spacetype == SPACE_VIEW3D &&
           data->region->regiontype == RGN_TYPE_WINDOW);
}

static void button_nuru_view3d_redraw_throttle_sync(HandleButtonData *data,
                                                    const HandleButtonState state)
{
  if (data->window == nullptr || data->window->runtime == nullptr) {
    data->throttles_nuru_view3d_redraw = false;
    return;
  }

  const bool should_throttle = button_state_throttles_nuru_view3d_redraw(state) &&
                               button_interaction_is_non_view3d_ui(data);
  if (data->throttles_nuru_view3d_redraw == should_throttle) {
    return;
  }

  blender::bke::WindowRuntime *win_runtime = data->window->runtime;
  if (should_throttle) {
    win_runtime->non_view3d_ui_interaction_count++;
    win_runtime->next_nuru_view3d_redraw_time = 0.0;
  }
  else {
    BLI_assert(win_runtime->non_view3d_ui_interaction_count > 0);
    win_runtime->non_view3d_ui_interaction_count = std::max(
        0, win_runtime->non_view3d_ui_interaction_count - 1);
    if (win_runtime->non_view3d_ui_interaction_count == 0) {
      win_runtime->next_nuru_view3d_redraw_time = 0.0;
    }
  }

  data->throttles_nuru_view3d_redraw = should_throttle;
}
...
static void button_activate_state(bContext *C, Button *but, HandleButtonState state)
{
  ...
  button_nuru_view3d_redraw_throttle_sync(data, state);
