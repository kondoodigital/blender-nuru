...
/** True when Eevee uses Nuru hardware ray tracing (not screen-space tracing). */
bool BKE_scene_eevee_uses_hardware_raytracing(const Scene *scene);
/** True for shader/material preview renders (`R_BUTS_PREVIEW`). */
bool BKE_scene_is_eevee_shader_preview(const Scene *scene);
/** Force screen-space Eevee ray tracing and disable hardware ray-tracing features. */
