...
bool BKE_scene_eevee_uses_hardware_raytracing(const Scene *scene)
{
  if (scene == nullptr || !BKE_scene_uses_blender_eevee(scene)) {
    return false;
  }
  return scene->eevee.ray_tracing_method == RAYTRACE_EEVEE_METHOD_HARDWARE;
}

bool BKE_scene_is_eevee_shader_preview(const Scene *scene)
{
  return scene != nullptr && (scene->r.scemode & R_BUTS_PREVIEW) != 0;
}

void BKE_scene_eevee_force_classic_raytracing(Scene *scene)
{
  if (scene == nullptr || !BKE_scene_uses_blender_eevee(scene)) {
    return;
  }

  SceneEEVEE &eevee = scene->eevee;
  eevee.ray_tracing_method = RAYTRACE_EEVEE_METHOD_SCREEN;
  eevee.hardware_raytracing_gi_mode = RAYTRACE_EEVEE_GI_MODE_OFF;
  eevee.hardware_raytracing_reflection_mode = RAYTRACE_EEVEE_SPECULAR_MODE_OFF;
  eevee.hardware_raytracing_refraction_mode = RAYTRACE_EEVEE_SPECULAR_MODE_OFF;
  eevee.hardware_raytracing_features = 0;
  eevee.ray_tracing_options.flag &= ~RAYTRACE_EEVEE_USE_DENOISE;
}
