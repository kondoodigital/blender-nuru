...
void blo_do_versions_520(FileData * /*fd*/, Library * /*lib*/, Main *bmain)
{
  ...
  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 502, 18)) {
    for (Scene &scene : bmain->scenes) {
      if (scene.eevee.ray_tracing_gi_bounces == 0) {
        scene.eevee.ray_tracing_gi_bounces = 1;
      }
    }
  }
  ...
  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 502, 19)) {
    for (Scene &scene : bmain->scenes) {
      if (scene.eevee.hardware_raytracing_features == 0) {
        scene.eevee.hardware_raytracing_features = RAYTRACE_EEVEE_HARDWARE_GI;
      }
    }
  }
  ...
  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 502, 20)) {
    for (Scene &scene : bmain->scenes) {
      if (scene.eevee.ray_tracing_reflection_bounces == 0) {
        scene.eevee.ray_tracing_reflection_bounces = 2;
      }
      if (scene.eevee.ray_tracing_refraction_bounces == 0) {
        scene.eevee.ray_tracing_refraction_bounces = 2;
      }
    }
  }
  ...
  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 502, 22)) {
    for (Scene &scene : bmain->scenes) {
      if (scene.eevee.hardware_raytracing_features & RAYTRACE_EEVEE_HARDWARE_REFLECTIONS) {
        scene.eevee.hardware_raytracing_reflection_mode = RAYTRACE_EEVEE_SPECULAR_MODE_HYBRID;
      }
      if (scene.eevee.hardware_raytracing_features & RAYTRACE_EEVEE_HARDWARE_REFRACTIONS) {
        scene.eevee.hardware_raytracing_refraction_mode = RAYTRACE_EEVEE_SPECULAR_MODE_HYBRID;
      }
    }
  }
  ...
  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 502, 23)) {
    for (Scene &scene : bmain->scenes) {
      const bool reflection_enabled =
          (scene.eevee.hardware_raytracing_features & RAYTRACE_EEVEE_HARDWARE_REFLECTIONS) ||
          (scene.eevee.hardware_raytracing_reflection_mode != RAYTRACE_EEVEE_SPECULAR_MODE_OFF);

      if (reflection_enabled) {
        scene.eevee.hardware_raytracing_features |= RAYTRACE_EEVEE_HARDWARE_REFLECTIONS;
        scene.eevee.hardware_raytracing_reflection_mode = RAYTRACE_EEVEE_SPECULAR_MODE_FULL_RT;
      }
      else {
        scene.eevee.hardware_raytracing_features &= ~RAYTRACE_EEVEE_HARDWARE_REFLECTIONS;
        scene.eevee.hardware_raytracing_reflection_mode = RAYTRACE_EEVEE_SPECULAR_MODE_OFF;
  ...
  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 502, 24)) {
    for (Scene &scene : bmain->scenes) {
      if (scene.eevee.ray_tracing_caustics_samples == 0) {
        scene.eevee.ray_tracing_caustics_samples = 8;
      }
    }
  }
  ...
  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 502, 25)) {
    for (Scene &scene : bmain->scenes) {
      const bool gi_enabled = (scene.eevee.hardware_raytracing_features &
                               RAYTRACE_EEVEE_HARDWARE_GI) != 0;
      if (!gi_enabled) {
        scene.eevee.hardware_raytracing_gi_mode = RAYTRACE_EEVEE_GI_MODE_OFF;
      }
      else if (!ELEM(scene.eevee.hardware_raytracing_gi_mode,
                     RAYTRACE_EEVEE_GI_MODE_ACCURATE,
                     RAYTRACE_EEVEE_GI_MODE_REFINE))
      {
        scene.eevee.hardware_raytracing_gi_mode = RAYTRACE_EEVEE_GI_MODE_ACCURATE;
      }
    }
  }
  ...
  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 502, 26)) {
    for (Scene &scene : bmain->scenes) {
      const bool fast_gi_enabled = (scene.eevee.hardware_raytracing_indirect_flags &
                                    SCE_EEVEE_HWRT_FAST_GI) != 0;
      if (scene.eevee.hardware_raytracing_gi_mode == RAYTRACE_EEVEE_GI_MODE_OFF &&
          fast_gi_enabled)
      {
        scene.eevee.hardware_raytracing_gi_mode = RAYTRACE_EEVEE_GI_MODE_FAST;
      }
      else if (scene.eevee.hardware_raytracing_gi_mode == RAYTRACE_EEVEE_GI_MODE_REFINE) {
        scene.eevee.hardware_raytracing_indirect_flags |= SCE_EEVEE_HWRT_FAST_GI;
      }
      else {
        scene.eevee.hardware_raytracing_indirect_flags &= ~SCE_EEVEE_HWRT_FAST_GI;
      }
