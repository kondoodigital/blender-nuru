...
void blo_do_versions_510(FileData *fd, Library * /*lib*/, Main *bmain)
{
  ...
  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 501, 25)) {
    for (Scene &scene : bmain->scenes) {
      ...
      scene.eevee.indirect_light_intensity = 5.0f;
    }
  }
  ...
  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 502, 31)) {
    for (Scene &scene : bmain->scenes) {
      scene.eevee.hardware_raytracing_shadow_transparency = 0.5f;
    }
  }

  if (!DNA_struct_member_exists(
          fd->filesdna, "SceneEEVEE", "float", "hardware_raytracing_shadow_color_intensity"))
  {
    for (Scene &scene : bmain->scenes) {
      scene.eevee.hardware_raytracing_shadow_color_intensity = 0.5f;
    }
  }

  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 502, 32)) {
    for (Scene &scene : bmain->scenes) {
      scene.eevee.hardware_raytracing_features &= ~RAYTRACE_EEVEE_HARDWARE_SHADOW_CAUSTICS;
    }
  }

  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 502, 33)) {
    for (Scene &scene : bmain->scenes) {
      RaytraceEEVEE &options = scene.eevee.ray_tracing_options;
      options.flag |= RAYTRACE_EEVEE_USE_DENOISE;
  ...
  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 502, 34)) {
    for (Scene &scene : bmain->scenes) {
      /* Nuru: Secondary GI (mirror receiver GI) ships enabled by default. */
      scene.eevee.hardware_raytracing_features |= RAYTRACE_EEVEE_HARDWARE_SECONDARY_GI;
    }
  }
