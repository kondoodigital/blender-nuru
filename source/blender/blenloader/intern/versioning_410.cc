  ...
  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 401, 10)) {
            ...
            fd->filesdna, "SceneEEVEE", "RaytraceEEVEE", "ray_tracing_options"))
    {
      for (Scene &scene : bmain->scenes) {
        ...
        scene.eevee.ray_tracing_options.denoise_filter = RAYTRACE_EEVEE_DENOISE_FILTER_OIDN;
        scene.eevee.ray_tracing_options.denoise_quality = RAYTRACE_EEVEE_DENOISE_QUALITY_BALANCED;
        scene.eevee.ray_tracing_options.denoise_input_passes =
            RAYTRACE_EEVEE_DENOISE_INPUT_RGB_ALBEDO_NORMAL;
        scene.eevee.ray_tracing_options.denoise_prefilter = RAYTRACE_EEVEE_DENOISE_PREFILTER_FAST;
        scene.eevee.ray_tracing_options.denoise_use_gpu = 1;
        scene.eevee.ray_tracing_options.denoise_sample_interval =
            RAYTRACE_EEVEE_DENOISE_SAMPLE_INTERVAL_1;
        scene.eevee.ray_tracing_options.gi_spatial_samples = RAYTRACE_EEVEE_GI_SPATIAL_16;
        scene.eevee.ray_tracing_options.screen_trace_quality = 0.25f;
        scene.eevee.ray_tracing_options.screen_trace_thickness = 0.2f;
        scene.eevee.ray_tracing_options.trace_max_roughness = 0.5f;
        scene.eevee.ray_tracing_options.resolution_scale = RAYTRACE_EEVEE_RESOLUTION_SCALE_50;
      }
    }

    if (!DNA_struct_member_exists(fd->filesdna, "RaytraceEEVEE", "int", "denoise_filter")) {
      for (Scene &scene : bmain->scenes) {
        scene.eevee.ray_tracing_options.denoise_stages &= (RAYTRACE_EEVEE_DENOISE_SPATIAL |
                                                           RAYTRACE_EEVEE_DENOISE_TEMPORAL |
                                                           RAYTRACE_EEVEE_DENOISE_BILATERAL);
    ...
    if (!DNA_struct_member_exists(fd->filesdna, "RaytraceEEVEE", "int", "denoise_use_gpu")) {
      for (Scene &scene : bmain->scenes) {
        scene.eevee.ray_tracing_options.denoise_use_gpu = 1;
      }
    }

    if (!DNA_struct_member_exists(fd->filesdna, "RaytraceEEVEE", "int", "denoise_sample_interval"))
    {
      for (Scene &scene : bmain->scenes) {
        scene.eevee.ray_tracing_options.denoise_sample_interval =
            RAYTRACE_EEVEE_DENOISE_SAMPLE_INTERVAL_1;
    ...
    if (!DNA_struct_member_exists(fd->filesdna, "RaytraceEEVEE", "int", "gi_spatial_samples")) {
      for (Scene &scene : bmain->scenes) {
        scene.eevee.ray_tracing_options.gi_spatial_samples = RAYTRACE_EEVEE_GI_SPATIAL_16;
      }
    }
    ...
    if (!DNA_struct_member_exists(
            fd->filesdna, "SceneEEVEE", "int", "hardware_raytracing_shadow_samples"))
    {
      for (Scene &scene : bmain->scenes) {
        scene.eevee.hardware_raytracing_shadow_samples = 2;
      }
    }
