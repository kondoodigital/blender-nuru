...
static void rna_def_light(BlenderRNA *brna)
{
  ...
  prop = RNA_def_property(srna, "visible_in_reflections", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "mode", LA_VISIBLE_REFLECTIONS);
  RNA_def_property_ui_text(prop,
                           "Visible in Reflections",
                           "Render this light's image in sharp (mirror-like) reflections. "
                           "Large lights otherwise show their shape in every mirror. Rough "
                           "glossy highlights are not affected by this toggle");
  RNA_def_property_update(prop, 0, "rna_Light_update");
