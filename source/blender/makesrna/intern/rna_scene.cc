...
static const EnumPropertyItem eevee_raytrace_resolution_percentage_items[] = {
    {RAYTRACE_EEVEE_RESOLUTION_SCALE_100,
     "PERCENT_100",
     0,
     "100%",
     "Render ray-traced GI at full resolution"},
    {RAYTRACE_EEVEE_RESOLUTION_SCALE_75,
     "PERCENT_75",
     0,
     "75%",
     "Nuru: render ray-traced GI at 75% render resolution (true 3/4 size, between 100% and 50%)"},
    {RAYTRACE_EEVEE_RESOLUTION_SCALE_50,
     "PERCENT_50",
     0,
     "50%",
     "Render ray-traced GI at 50% render resolution"},
    {RAYTRACE_EEVEE_RESOLUTION_SCALE_25,
     "PERCENT_25",
     0,
     "25%",
     "Render ray-traced GI at 25% render resolution"},
    {0, nullptr, 0, nullptr, nullptr},
};

static const EnumPropertyItem eevee_denoise_filter_items[] = {
    {RAYTRACE_EEVEE_DENOISE_FILTER_OIDN,
     "OIDN",
     0,
     "OpenImageDenoise",
     "Use the GPU OpenImageDenoise filter for the final Hardware RT ray result"},
    {RAYTRACE_EEVEE_DENOISE_FILTER_BILATERAL,
     "BILATERAL",
     0,
     "Bilateral",
     "Use the built-in bilateral filter for the final Hardware RT ray result"},
    {0, nullptr, 0, nullptr, nullptr},
};
...
static const EnumPropertyItem eevee_denoise_input_pass_items[] = {
    {RAYTRACE_EEVEE_DENOISE_INPUT_RGB,
     ...
     0,
     "None",
     "Do not use utility passes for denoising"},
    {RAYTRACE_EEVEE_DENOISE_INPUT_RGB_ALBEDO,
     "RGB_ALBEDO",
     0,
     "Albedo",
     "Use albedo to help the denoiser preserve material detail"},
    {RAYTRACE_EEVEE_DENOISE_INPUT_RGB_ALBEDO_NORMAL,
     "RGB_ALBEDO_NORMAL",
     0,
     "Albedo and Normal",
     "Use albedo and normal to help the denoiser preserve material and geometry detail"},
    {0, nullptr, 0, nullptr, nullptr},
};

static const EnumPropertyItem eevee_denoise_prefilter_items[] = {
    {RAYTRACE_EEVEE_DENOISE_PREFILTER_NONE,
     "NONE",
     0,
     "None",
     "No prefiltering, for clean guiding passes"},
    {RAYTRACE_EEVEE_DENOISE_PREFILTER_FAST,
     "FAST",
     0,
     "Fast",
     "Denoise color and guiding passes together with the least extra processing"},
    {RAYTRACE_EEVEE_DENOISE_PREFILTER_ACCURATE,
     "ACCURATE",
     0,
     "Accurate",
     "Use stronger auxiliary pass handling for cleaner denoising"},
    {0, nullptr, 0, nullptr, nullptr},
};

static const EnumPropertyItem eevee_denoise_quality_items[] = {
    {RAYTRACE_EEVEE_DENOISE_QUALITY_HIGH, "HIGH", 0, "High", "High quality"},
    {RAYTRACE_EEVEE_DENOISE_QUALITY_BALANCED,
     "BALANCED",
     0,
     "Balanced",
     "Balanced between performance and quality"},
    {RAYTRACE_EEVEE_DENOISE_QUALITY_FAST, "FAST", 0, "Fast", "High performance"},
    {0, nullptr, 0, nullptr, nullptr},
};

static const EnumPropertyItem eevee_denoise_sample_interval_items[] = {
    {RAYTRACE_EEVEE_DENOISE_SAMPLE_INTERVAL_1,
     "1",
     0,
     "1",
     "Run OpenImageDenoise every sample for maximum coherence"},
    {RAYTRACE_EEVEE_DENOISE_SAMPLE_INTERVAL_2,
     "2",
     0,
     "2",
     "Run OpenImageDenoise every second sample"},
    {RAYTRACE_EEVEE_DENOISE_SAMPLE_INTERVAL_4,
     "4",
     0,
     "4",
     "Run OpenImageDenoise every fourth sample"},
    {RAYTRACE_EEVEE_DENOISE_SAMPLE_INTERVAL_8,
     "8",
     0,
     "8",
     "Run OpenImageDenoise every eighth sample"},
    {RAYTRACE_EEVEE_DENOISE_SAMPLE_INTERVAL_16,
     "16",
     0,
     "16",
     "Run OpenImageDenoise every sixteenth sample"},
    {0, nullptr, 0, nullptr, nullptr},
};

static const EnumPropertyItem eevee_gi_spatial_sample_items[] = {
    {RAYTRACE_EEVEE_GI_SPATIAL_8,
     "8",
     0,
     "8",
     "Trace 8 diffuse GI rays per low-resolution gather point"},
    {RAYTRACE_EEVEE_GI_SPATIAL_16,
     "16",
     0,
     "16",
     "Trace 16 diffuse GI rays per low-resolution gather point"},
    {RAYTRACE_EEVEE_GI_SPATIAL_32,
     "32",
     0,
     "32",
     "Trace 32 diffuse GI rays per low-resolution gather point"},
    {0, nullptr, 0, nullptr, nullptr},
};
...
static void rna_SceneEEVEE_ray_tracing_method_update(Main * /*main*/,
                                                     Scene *scene,
                                                     PointerRNA * /*ptr*/)
{
  DEG_id_tag_update(&scene->id, ID_RECALC_SYNC_TO_EVAL);
...
static int rna_SceneEEVEE_specular_mode_sanitize(int value)
{
...
static void rna_SceneEEVEE_specular_mode_assign(SceneEEVEE *eevee,
                                                int *mode_field,
                                                const int feature_flag,
                                                int value)
{
  value = rna_SceneEEVEE_specular_mode_sanitize(value);
  *mode_field = value;
  if (value == RAYTRACE_EEVEE_SPECULAR_MODE_OFF) {
    eevee->hardware_raytracing_features &= ~feature_flag;
  }
  else {
    eevee->hardware_raytracing_features |= feature_flag;
  }
...
static int rna_SceneEEVEE_reflection_mode_sanitize(const int value)
{
  const int sanitized = rna_SceneEEVEE_specular_mode_sanitize(value);
  return (sanitized == RAYTRACE_EEVEE_SPECULAR_MODE_OFF) ? RAYTRACE_EEVEE_SPECULAR_MODE_OFF :
                                                           RAYTRACE_EEVEE_SPECULAR_MODE_FULL_RT;
}

static int rna_SceneEEVEE_refraction_mode_sanitize(const int value)
{
  const int sanitized = rna_SceneEEVEE_specular_mode_sanitize(value);
  switch (sanitized) {
    case RAYTRACE_EEVEE_SPECULAR_MODE_FULL_RT:
      return RAYTRACE_EEVEE_SPECULAR_MODE_FULL_RT;
    case RAYTRACE_EEVEE_SPECULAR_MODE_OFF:
      return RAYTRACE_EEVEE_SPECULAR_MODE_OFF;
    case RAYTRACE_EEVEE_SPECULAR_MODE_AUTO:
    case RAYTRACE_EEVEE_SPECULAR_MODE_HYBRID:
    default:
      return RAYTRACE_EEVEE_SPECULAR_MODE_HYBRID;
  }
}

static int rna_SceneEEVEE_gi_mode_sanitize(const int value)
{
  switch (value) {
    case RAYTRACE_EEVEE_GI_MODE_ACCURATE:
    case RAYTRACE_EEVEE_GI_MODE_OFF:
      return value;
    case RAYTRACE_EEVEE_GI_MODE_REFINE:
    case RAYTRACE_EEVEE_GI_MODE_FAST:
      return RAYTRACE_EEVEE_GI_MODE_ACCURATE;
    default:
      return RAYTRACE_EEVEE_GI_MODE_ACCURATE;
  }
}

static int rna_RaytraceEEVEE_resolution_percentage_sanitize(const int value)
{
  switch (value) {
    ...
    case RAYTRACE_EEVEE_RESOLUTION_SCALE_25:
      return value;
    default:
      return RAYTRACE_EEVEE_RESOLUTION_SCALE_50;
  }
}

static int rna_RaytraceEEVEE_resolution_percentage_get(PointerRNA *ptr)
{
  const RaytraceEEVEE *options = static_cast<const RaytraceEEVEE *>(ptr->data);
  const float value = float(options->resolution_scale);
  const int percentage_items[] = {
      RAYTRACE_EEVEE_RESOLUTION_SCALE_100,
      RAYTRACE_EEVEE_RESOLUTION_SCALE_75,
      RAYTRACE_EEVEE_RESOLUTION_SCALE_50,
      RAYTRACE_EEVEE_RESOLUTION_SCALE_25,
  };
  int best = RAYTRACE_EEVEE_RESOLUTION_SCALE_50;
  float best_distance = FLT_MAX;
  for (const int percentage : percentage_items) {
    const float distance = std::abs(value - float(percentage));
...
static void rna_RaytraceEEVEE_resolution_percentage_set(PointerRNA *ptr, int value)
{
  RaytraceEEVEE *options = static_cast<RaytraceEEVEE *>(ptr->data);
  options->resolution_scale = rna_RaytraceEEVEE_resolution_percentage_sanitize(value);
}

static int rna_RaytraceEEVEE_gi_spatial_samples_sanitize(const int value)
{
  switch (value) {
...
static int rna_RaytraceEEVEE_gi_spatial_samples_get(PointerRNA *ptr)
{
  const RaytraceEEVEE *options = static_cast<const RaytraceEEVEE *>(ptr->data);
  return rna_RaytraceEEVEE_gi_spatial_samples_sanitize(options->gi_spatial_samples);
}

static void rna_RaytraceEEVEE_gi_spatial_samples_set(PointerRNA *ptr, int value)
{
  RaytraceEEVEE *options = static_cast<RaytraceEEVEE *>(ptr->data);
  options->gi_spatial_samples = rna_RaytraceEEVEE_gi_spatial_samples_sanitize(value);
}

static int rna_RaytraceEEVEE_denoise_sample_interval_sanitize(const int value)
{
  switch (value) {
...
static int rna_RaytraceEEVEE_denoise_sample_interval_get(PointerRNA *ptr)
{
  const RaytraceEEVEE *options = static_cast<const RaytraceEEVEE *>(ptr->data);
  return rna_RaytraceEEVEE_denoise_sample_interval_sanitize(options->denoise_sample_interval);
}

static void rna_RaytraceEEVEE_denoise_sample_interval_set(PointerRNA *ptr, int value)
{
  RaytraceEEVEE *options = static_cast<RaytraceEEVEE *>(ptr->data);
  options->denoise_sample_interval = rna_RaytraceEEVEE_denoise_sample_interval_sanitize(value);
}

static int rna_SceneEEVEE_hardware_gi_mode_resolve(const SceneEEVEE *eevee)
{
  return rna_SceneEEVEE_gi_mode_sanitize(eevee->hardware_raytracing_gi_mode);
}

static void rna_SceneEEVEE_hardware_gi_mode_assign(SceneEEVEE *eevee, const int value)
{
  const int mode = rna_SceneEEVEE_gi_mode_sanitize(value);
  eevee->hardware_raytracing_gi_mode = mode;
  eevee->hardware_raytracing_indirect_flags &= ~SCE_EEVEE_HWRT_FAST_GI;
  if (mode == RAYTRACE_EEVEE_GI_MODE_OFF) {
    eevee->hardware_raytracing_features &= ~RAYTRACE_EEVEE_HARDWARE_GI;
  }
  else {
    eevee->hardware_raytracing_features |= RAYTRACE_EEVEE_HARDWARE_GI;
  }
...
static int rna_SceneEEVEE_hardware_gi_mode_get(PointerRNA *ptr)
{
  const SceneEEVEE *eevee = static_cast<const SceneEEVEE *>(ptr->data);
  return rna_SceneEEVEE_hardware_gi_mode_resolve(eevee);
}

static void rna_SceneEEVEE_hardware_gi_mode_set(PointerRNA *ptr, int value)
{
  SceneEEVEE *eevee = static_cast<SceneEEVEE *>(ptr->data);
  rna_SceneEEVEE_hardware_gi_mode_assign(eevee, value);
}

static bool rna_SceneEEVEE_use_hardware_raytracing_gi_get(PointerRNA *ptr)
{
  return rna_SceneEEVEE_hardware_gi_mode_get(ptr) == RAYTRACE_EEVEE_GI_MODE_ACCURATE;
}

static void rna_SceneEEVEE_use_hardware_raytracing_gi_set(PointerRNA *ptr, bool value)
{
  rna_SceneEEVEE_hardware_gi_mode_set(
      ptr, value ? RAYTRACE_EEVEE_GI_MODE_ACCURATE : RAYTRACE_EEVEE_GI_MODE_OFF);
}

static int rna_SceneEEVEE_hardware_reflection_mode_get(PointerRNA *ptr)
{
  const SceneEEVEE *eevee = static_cast<const SceneEEVEE *>(ptr->data);
  return rna_SceneEEVEE_reflection_mode_sanitize(eevee->hardware_raytracing_reflection_mode);
}

static void rna_SceneEEVEE_hardware_reflection_mode_set(PointerRNA *ptr, int value)
{
  SceneEEVEE *eevee = static_cast<SceneEEVEE *>(ptr->data);
  rna_SceneEEVEE_specular_mode_assign(
      eevee,
      &eevee->hardware_raytracing_reflection_mode,
      RAYTRACE_EEVEE_HARDWARE_REFLECTIONS,
      rna_SceneEEVEE_reflection_mode_sanitize(value));
}

static int rna_SceneEEVEE_hardware_refraction_mode_get(PointerRNA *ptr)
{
  const SceneEEVEE *eevee = static_cast<const SceneEEVEE *>(ptr->data);
  return rna_SceneEEVEE_refraction_mode_sanitize(eevee->hardware_raytracing_refraction_mode);
}

static void rna_SceneEEVEE_hardware_refraction_mode_set(PointerRNA *ptr, int value)
{
  SceneEEVEE *eevee = static_cast<SceneEEVEE *>(ptr->data);
  rna_SceneEEVEE_specular_mode_assign(
      eevee,
      &eevee->hardware_raytracing_refraction_mode,
      RAYTRACE_EEVEE_HARDWARE_REFRACTIONS,
      rna_SceneEEVEE_refraction_mode_sanitize(value));
}

static bool rna_SceneEEVEE_use_hardware_raytracing_reflections_get(PointerRNA *ptr)
{
  return rna_SceneEEVEE_hardware_reflection_mode_get(ptr) != RAYTRACE_EEVEE_SPECULAR_MODE_OFF;
}

static void rna_SceneEEVEE_use_hardware_raytracing_reflections_set(PointerRNA *ptr, bool value)
{
  rna_SceneEEVEE_hardware_reflection_mode_set(
      ptr, value ? RAYTRACE_EEVEE_SPECULAR_MODE_FULL_RT : RAYTRACE_EEVEE_SPECULAR_MODE_OFF);
}

static bool rna_SceneEEVEE_use_hardware_raytracing_refractions_get(PointerRNA *ptr)
{
  return rna_SceneEEVEE_hardware_refraction_mode_get(ptr) != RAYTRACE_EEVEE_SPECULAR_MODE_OFF;
}

static void rna_SceneEEVEE_use_hardware_raytracing_refractions_set(PointerRNA *ptr, bool value)
{
  rna_SceneEEVEE_hardware_refraction_mode_set(
      ptr, value ? RAYTRACE_EEVEE_SPECULAR_MODE_FULL_RT : RAYTRACE_EEVEE_SPECULAR_MODE_OFF);
}
...
static bool rna_SceneEEVEE_is_hardware_raytracing_supported_get(PointerRNA * /*ptr*/)
{
  return GPU_viewport_hardware_raytracing_support();
}

static const char *rna_SceneEEVEE_hardware_raytracing_support_label_string()
{
  if (!GPU_viewport_hardware_raytracing_support()) {
    return "Raytracing unavailable";
  }
  if (GPU_type_matches(GPU_DEVICE_APPLE, GPU_OS_ANY, GPU_DRIVER_ANY)) {
    return "Apple Raytracing detected";
  }
  if (GPU_type_matches(GPU_DEVICE_NVIDIA, GPU_OS_ANY, GPU_DRIVER_ANY)) {
    return "Nvidia Raytracing detected";
  }
  if (GPU_type_matches(GPU_DEVICE_ATI, GPU_OS_ANY, GPU_DRIVER_ANY)) {
    return "AMD Raytracing detected";
  }
  if (GPU_type_matches(GPU_DEVICE_INTEL | GPU_DEVICE_INTEL_UHD, GPU_OS_ANY, GPU_DRIVER_ANY)) {
    return "Intel Raytracing detected";
  }

  const char *vendor = GPU_platform_vendor();
  if (vendor != nullptr && vendor[0] != '\0') {
    return vendor;
  }
  return "Raytracing detected";
}

static void rna_SceneEEVEE_hardware_raytracing_support_label_get(PointerRNA * /*ptr*/, char *value)
{
  const char *label = rna_SceneEEVEE_hardware_raytracing_support_label_string();
  memcpy(value, label, strlen(label) + 1);
}

static int rna_SceneEEVEE_hardware_raytracing_support_label_length(PointerRNA * /*ptr*/)
{
  return strlen(rna_SceneEEVEE_hardware_raytracing_support_label_string());
}
...
static void rna_def_raytrace_eevee(BlenderRNA *brna)
{
  ...
  RNA_def_property_enum_items(prop, eevee_raytrace_resolution_percentage_items);
  RNA_def_property_enum_funcs(prop,
                              "rna_RaytraceEEVEE_resolution_percentage_get",
                              "rna_RaytraceEEVEE_resolution_percentage_set",
                              nullptr);
  RNA_def_property_ui_text(prop,
                           "Resolution",
                           "Percentage of render resolution used by the ray-traced GI grid");
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "gi_spatial_samples", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_items(prop, eevee_gi_spatial_sample_items);
  RNA_def_property_enum_funcs(prop,
                              "rna_RaytraceEEVEE_gi_spatial_samples_get",
                              "rna_RaytraceEEVEE_gi_spatial_samples_set",
                              nullptr);
  RNA_def_property_ui_text(
      prop,
      "GI Spatial",
      "Diffuse Hardware RT GI rays per low-resolution gather point before OIDN reconstruction");
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);
  ...
  RNA_def_property_ui_text(
      prop,
      "Denoise",
      "Deprecated in Nuru: OpenImageDenoise is always enabled for Hardware RT");
  RNA_def_property_flag(prop, PROP_HIDDEN);
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "denoise_filter", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, nullptr, "denoise_filter");
  RNA_def_property_enum_items(prop, eevee_denoise_filter_items);
  RNA_def_property_ui_text(
      prop,
      "Denoiser",
  ...
  prop = RNA_def_property(srna, "denoise_input_passes", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, nullptr, "denoise_input_passes");
  RNA_def_property_enum_items(prop, eevee_denoise_input_pass_items);
  RNA_def_property_ui_text(
      prop, "Denoising Input Passes", "Auxiliary passes used by OpenImageDenoise");
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "denoise_prefilter", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, nullptr, "denoise_prefilter");
  RNA_def_property_enum_items(prop, eevee_denoise_prefilter_items);
  RNA_def_property_ui_text(
      prop, "Denoising Prefilter", "Prefilter noisy auxiliary passes for OpenImageDenoise");
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "denoise_quality", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, nullptr, "denoise_quality");
  RNA_def_property_enum_items(prop, eevee_denoise_quality_items);
  RNA_def_property_ui_text(prop, "Denoising Quality", "Overall OpenImageDenoise quality");
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "denoise_sample_interval", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_items(prop, eevee_denoise_sample_interval_items);
  RNA_def_property_enum_funcs(prop,
                              "rna_RaytraceEEVEE_denoise_sample_interval_get",
                              "rna_RaytraceEEVEE_denoise_sample_interval_set",
                              nullptr);
  RNA_def_property_ui_text(prop,
                           "Denoise Sample Interval",
                           "Run OpenImageDenoise every N samples while keeping "
                           "spatial reconstruction active every sample");
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "denoise_use_gpu", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "denoise_use_gpu", 1);
  RNA_def_property_ui_text(
      prop,
      "Denoise on GPU",
      "Use the Metal GPU OpenImageDenoise backend. When disabled, use the CPU "
      "OpenImageDenoise backend on the same ray buffer");
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);
  ...
  RNA_def_property_ui_text(prop, "Bilateral Filter", "Blur noisy ray-traced radiance");
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);
...
static void rna_def_scene_eevee(BlenderRNA *brna)
{
  ...
  static const EnumPropertyItem ray_tracing_method_items[] = {
      ...
      {RAYTRACE_EEVEE_METHOD_HARDWARE,
       "HARDWARE",
       0,
       "Nuru Raytracing",
       "Nuru per-feature scene-intersection ray tracing backend. Disabled Nuru raytracing "
       "features keep using classic Eevee methods until their dedicated backend is available"},
      {0, nullptr, 0, nullptr, nullptr},
  };

  static const EnumPropertyItem hardware_reflection_mode_items[] = {
      {RAYTRACE_EEVEE_SPECULAR_MODE_OFF,
       "OFF",
       0,
       "Off",
       "Keep classic Eevee ownership for reflections"},
      {RAYTRACE_EEVEE_SPECULAR_MODE_FULL_RT,
       "FULL",
       0,
       "On",
       "Let Hardware RT own reflections fully"},
      {0, nullptr, 0, nullptr, nullptr},
  };
  static const EnumPropertyItem hardware_refraction_mode_items[] = {
      {RAYTRACE_EEVEE_SPECULAR_MODE_OFF,
       "OFF",
       0,
       "Off",
       "Keep classic Eevee ownership for refractions"},
      {RAYTRACE_EEVEE_SPECULAR_MODE_HYBRID,
       "HYBRID",
       0,
       "Hybrid",
       "Use screen or planar first-hit ownership when available, and continue or fill with "
       "Hardware RT"},
      {RAYTRACE_EEVEE_SPECULAR_MODE_FULL_RT,
       "FULL",
       0,
       "On",
       "Let Hardware RT own refractions fully"},
      {0, nullptr, 0, nullptr, nullptr},
  };

  static const EnumPropertyItem hardware_gi_mode_items[] = {
      {RAYTRACE_EEVEE_GI_MODE_OFF,
       "OFF",
       0,
       "Off",
       "Disable Global Illumination diffuse transport"},
      {RAYTRACE_EEVEE_GI_MODE_ACCURATE,
       "ON",
       0,
       "On",
       "Enable Global Illumination diffuse transport"},
      {0, nullptr, 0, nullptr, nullptr},
  };
  ...
  RNA_def_property_update(
      prop, NC_SCENE | ND_RENDER_OPTIONS, "rna_SceneEEVEE_ray_tracing_method_update");

  prop = RNA_def_property(srna, "ray_tracing_reflection_bounces", PROP_INT, PROP_UNSIGNED);
  RNA_def_property_int_sdna(prop, nullptr, "ray_tracing_reflection_bounces");
  RNA_def_property_range(prop, 1, 8);
  RNA_def_property_ui_text(prop,
                           "Hardware Reflection Bounces",
                           "Maximum number of reflection bounces for Full RT Hardware reflections "
                           "(up to 8; Hybrid uses a lower continuation cap)");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "ray_tracing_refraction_bounces", PROP_INT, PROP_UNSIGNED);
  RNA_def_property_int_sdna(prop, nullptr, "ray_tracing_refraction_bounces");
  RNA_def_property_range(prop, 1, 8);
  RNA_def_property_ui_text(prop,
                           "Hardware Refraction Bounces",
                           "Maximum number of refraction bounces for Full RT Hardware refractions "
                           "(up to 8; Hybrid uses a lower continuation cap)");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "use_hardware_raytracing_gi", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_funcs(prop,
                                 "rna_SceneEEVEE_use_hardware_raytracing_gi_get",
                                 "rna_SceneEEVEE_use_hardware_raytracing_gi_set");
  RNA_def_property_ui_text(prop,
                           "Global Illumination",
                           "Enable or disable Global Illumination diffuse transport");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "hardware_raytracing_gi_mode", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_items(prop, hardware_gi_mode_items);
  RNA_def_property_enum_funcs(prop,
                              "rna_SceneEEVEE_hardware_gi_mode_get",
                              "rna_SceneEEVEE_hardware_gi_mode_set",
                              nullptr);
  RNA_def_property_ui_text(prop,
                           "Global Illumination Mode",
                           "Choose whether Global Illumination diffuse transport is off or on");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "use_hardware_raytracing_secondary_gi", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(
      prop, nullptr, "hardware_raytracing_features", RAYTRACE_EEVEE_HARDWARE_SECONDARY_GI);
  RNA_def_property_ui_text(
      prop,
      "Secondary GI",
      "Light diffuse surfaces seen through Hardware RT reflections and refractions with the "
      "world-space traced GI field (mirror interiors). Builds only while reflections or "
      "refractions are enabled");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "use_hardware_raytracing_shadows", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(
      prop, nullptr, "hardware_raytracing_features", RAYTRACE_EEVEE_HARDWARE_SHADOWS);
  RNA_def_property_ui_text(prop,
                           "Hardware Shadows",
                           "Use the experimental Hardware RT backend for shadows. "
                           "When disabled, classic Eevee shadowing stays active");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "hardware_raytracing_shadow_samples", PROP_INT, PROP_UNSIGNED);
  RNA_def_property_int_sdna(prop, nullptr, "hardware_raytracing_shadow_samples");
  RNA_def_property_range(prop, 1, 16);
  RNA_def_property_ui_text(prop,
                           "Hardware Shadow Samples",
                           "Kernel samples per Hardware RT shadow visibility evaluation");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "hardware_raytracing_shadow_transparency", PROP_FLOAT, PROP_FACTOR);
  RNA_def_property_float_sdna(prop, nullptr, "hardware_raytracing_shadow_transparency");
  RNA_def_property_range(prop, 0.0f, 1.0f);
  RNA_def_property_ui_range(prop, 0.0f, 1.0f, 1, 2);
  RNA_def_property_ui_text(
      prop,
      "Transparent Shadows",
      "Blend Hardware RT shadows between fully opaque (0) and transparent transmission "
      "tracing (1)");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  /* Nuru: saturation knob on the tinted transmission shadow baseline. */
  prop = RNA_def_property(srna, "hardware_raytracing_shadow_color_intensity", PROP_FLOAT, PROP_FACTOR);
  RNA_def_property_float_sdna(prop, nullptr, "hardware_raytracing_shadow_color_intensity");
  RNA_def_property_range(prop, 0.0f, 1.0f);
  RNA_def_property_ui_range(prop, 0.0f, 1.0f, 1, 2);
  RNA_def_property_ui_text(
      prop,
      "Color Transmission",
      "Saturation of the tinted transmission shadow under refractive materials. "
      "0 desaturates to gray, 1 uses the glass transmission color");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "hardware_raytracing_reflection_mode", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_items(prop, hardware_reflection_mode_items);
  RNA_def_property_enum_funcs(prop,
                              "rna_SceneEEVEE_hardware_reflection_mode_get",
                              "rna_SceneEEVEE_hardware_reflection_mode_set",
                              nullptr);
  RNA_def_property_ui_text(prop,
                           "Hardware Reflection Mode",
                           "Choose whether reflections stay on classic Eevee or let Hardware RT own "
                           "them fully");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "use_hardware_raytracing_reflections", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_funcs(prop,
                                 "rna_SceneEEVEE_use_hardware_raytracing_reflections_get",
                                 "rna_SceneEEVEE_use_hardware_raytracing_reflections_set");
  RNA_def_property_ui_text(prop,
                           "Hardware Reflections",
                           "Compatibility toggle for scripts. Enabling sets the reflection mode to "
                           "Full RT and disabling sets it to Off");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "hardware_raytracing_refraction_mode", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_items(prop, hardware_refraction_mode_items);
  RNA_def_property_enum_funcs(prop,
                              "rna_SceneEEVEE_hardware_refraction_mode_get",
                              "rna_SceneEEVEE_hardware_refraction_mode_set",
                              nullptr);
  RNA_def_property_ui_text(prop,
                           "Hardware Refraction Mode",
                           "Choose whether refractions stay on classic Eevee, use a hybrid screen "
                           "plus Hardware RT path, or let Hardware RT own them fully");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "use_hardware_raytracing_refractions", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_funcs(prop,
                                 "rna_SceneEEVEE_use_hardware_raytracing_refractions_get",
                                 "rna_SceneEEVEE_use_hardware_raytracing_refractions_set");
  RNA_def_property_ui_text(prop,
                           "Hardware Refractions",
                           "Compatibility toggle for scripts. Enabling sets the refraction mode "
                           "to Full RT and disabling sets it to Off");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "use_hardware_raytracing_environment", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(
      prop, nullptr, "hardware_raytracing_features", RAYTRACE_EEVEE_HARDWARE_ENVIRONMENT);
  RNA_def_property_ui_text(prop,
                           "Deprecated Hardware Environment",
                           "Deprecated compatibility property. World misses are owned by Hardware "
                           "RT reflections and refractions directly");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "is_hardware_raytracing_supported", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_funcs(
      prop, "rna_SceneEEVEE_is_hardware_raytracing_supported_get", nullptr);
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(prop,
                           "Viewport Hardware Raytracing Support",
                           "Whether the active viewport GPU backend reports hardware "
                           "ray-tracing support");

  prop = RNA_def_property(srna, "hardware_raytracing_support_label", PROP_STRING, PROP_NONE);
  RNA_def_property_string_funcs(prop,
                                "rna_SceneEEVEE_hardware_raytracing_support_label_get",
                                "rna_SceneEEVEE_hardware_raytracing_support_label_length",
                                nullptr);
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(prop,
                           "Hardware Raytracing Support Label",
                           "Human-readable label for the detected hardware raytracing backend");
  ...
  RNA_def_property_ui_text(prop,
                           "Distance",
                           "If zero, choose the fast GI field distance automatically from "
                           "scene density and camera framing. Non-zero values override that "
                           "automatic distance");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);
  ...
  RNA_def_property_ui_text(prop,
                           "Resolution",
                           "Quality hint for the automatic fast GI field layout. "
                           "Higher resolution uses more memory.");
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);
