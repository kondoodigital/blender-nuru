...
static void rna_SceneEEVEE_ray_tracing_method_update(Main *main,
                                                     ...
                                                     PointerRNA *ptr)
{
  /* The Eevee default indirect clamp is conservative for generic raster GI, but it makes
   * experimental Hardware RT emissive and HDRI intensity look artificially capped. When the user
   * switches to Hardware RT and the clamp is still at the untouched default, lift it. */
  if (scene->eevee.ray_tracing_method == RAYTRACE_EEVEE_METHOD_HARDWARE &&
      scene->eevee.clamp_surface_indirect == 10.0f)
  {
    scene->eevee.clamp_surface_indirect = 0.0f;
    rna_SceneEEVEE_clamp_surface_indirect_update(main, scene, ptr);
  }
...
static int rna_SceneEEVEE_indirect_gi_resolution_get(PointerRNA *ptr)
{
  const SceneEEVEE *eevee = static_cast<const SceneEEVEE *>(ptr->data);
  return rna_SceneEEVEE_indirect_gi_resolution_sanitize(
      eevee->hardware_raytracing_indirect_gi_resolution);
}

static void rna_SceneEEVEE_indirect_gi_resolution_set(PointerRNA *ptr, int value)
{
  SceneEEVEE *eevee = static_cast<SceneEEVEE *>(ptr->data);
  eevee->hardware_raytracing_indirect_gi_resolution =
      rna_SceneEEVEE_indirect_gi_resolution_sanitize(value);
}
...
static void rna_def_scene_eevee(BlenderRNA *brna)
{
  ...
  RNA_def_property_ui_text(prop,
                           ...
                           "Maximum number of reflection bounces when using the experimental "
                           "Hardware RT reflection feature");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);
  ...
  RNA_def_property_ui_text(prop,
                           ...
                           "Maximum number of refraction bounces when using the experimental "
                           "Hardware RT refraction feature");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);
  ...
  RNA_def_property_enum_items(prop, eevee_indirect_gi_resolution_items);
  RNA_def_property_enum_funcs(prop,
                              "rna_SceneEEVEE_indirect_gi_resolution_get",
  ...
  RNA_def_property_ui_text(prop,
                           ...
                           "Use the experimental Hardware RT backend for explicit environment "
                           "contribution and world-shadow transport. When disabled, Nuru Hardware "
                           "GI still keeps its own diffuse world transport");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);
