...
static void def_thin_glass(BlenderRNA * /*brna*/, StructRNA *srna)
{
  PropertyRNA *prop;
  ...
  RNA_def_property_boolean_negative_sdna(prop, nullptr, "custom2", 1);
  RNA_def_property_boolean_default(prop, true);
  RNA_def_property_ui_text(prop,
                           "Use Fresnel",
                           "Use the IOR to strengthen reflection at grazing angles");
  RNA_def_property_update(prop, NC_NODE | NA_EDITED, "rna_Node_update");
}
...
static void rna_def_nodes(BlenderRNA *brna)
{
  ...
  define("ShaderNode", "ShaderNodeBsdfThinGlass", def_thin_glass);
