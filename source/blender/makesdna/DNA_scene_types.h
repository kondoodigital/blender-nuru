...
enum RaytraceEEVEE_HardwareFeature {
  RAYTRACE_EEVEE_HARDWARE_GI = (1 << 0),
  RAYTRACE_EEVEE_HARDWARE_SHADOWS = (1 << 1),
  RAYTRACE_EEVEE_HARDWARE_REFLECTIONS = (1 << 2),
  RAYTRACE_EEVEE_HARDWARE_REFRACTIONS = (1 << 3),
  RAYTRACE_EEVEE_HARDWARE_ENVIRONMENT = (1 << 4),
  /* Nuru: curvature-based caustic focus factor applied inside the transparent-shadow trace so
   * refractive geometry (e.g. glass spheres) concentrates / spreads light onto receivers in the
   * shadow path. Independent of `HARDWARE_SHADOWS` so users can keep flat tinted shadows when
   * they don't want the extra cost. */
  RAYTRACE_EEVEE_HARDWARE_SHADOW_CAUSTICS = (1 << 5),
  /* Nuru: world-space Fast GI brick field lighting diffuse receivers seen through scene-final
   * Hardware reflections/refractions (mirror interiors). Independent of `HARDWARE_GI`, which
   * owns visible-surface diffuse transport. */
  RAYTRACE_EEVEE_HARDWARE_SECONDARY_GI = (1 << 6),
};
...
enum RaytraceEEVEE_DenoiseFilter {
  RAYTRACE_EEVEE_DENOISE_FILTER_BILATERAL = 0,
...
enum RaytraceEEVEE_DenoiseInputPasses {
  ...
  RAYTRACE_EEVEE_DENOISE_INPUT_RGB_ALBEDO_NORMAL = 3,
};

enum RaytraceEEVEE_DenoisePrefilter {
  RAYTRACE_EEVEE_DENOISE_PREFILTER_NONE = 1,
  RAYTRACE_EEVEE_DENOISE_PREFILTER_FAST = 2,
  RAYTRACE_EEVEE_DENOISE_PREFILTER_ACCURATE = 3,
};

enum RaytraceEEVEE_DenoiseQuality {
  RAYTRACE_EEVEE_DENOISE_QUALITY_HIGH = 1,
  RAYTRACE_EEVEE_DENOISE_QUALITY_BALANCED = 2,
  RAYTRACE_EEVEE_DENOISE_QUALITY_FAST = 3,
};

enum RaytraceEEVEE_DenoiseSampleInterval {
  RAYTRACE_EEVEE_DENOISE_SAMPLE_INTERVAL_1 = 1,
  RAYTRACE_EEVEE_DENOISE_SAMPLE_INTERVAL_2 = 2,
  RAYTRACE_EEVEE_DENOISE_SAMPLE_INTERVAL_4 = 4,
  RAYTRACE_EEVEE_DENOISE_SAMPLE_INTERVAL_8 = 8,
  RAYTRACE_EEVEE_DENOISE_SAMPLE_INTERVAL_16 = 16,
};

enum RaytraceEEVEE_GISpatialSamples {
  RAYTRACE_EEVEE_GI_SPATIAL_8 = 8,
  RAYTRACE_EEVEE_GI_SPATIAL_16 = 16,
  RAYTRACE_EEVEE_GI_SPATIAL_32 = 32,
};

enum RaytraceEEVEE_ResolutionScale {
  RAYTRACE_EEVEE_RESOLUTION_SCALE_100 = 100,
  /* Nuru: true 3/4 raytrace resolution. Numerator=4 / denominator=3 -> screen * 3/4. */
  RAYTRACE_EEVEE_RESOLUTION_SCALE_75 = 75,
  RAYTRACE_EEVEE_RESOLUTION_SCALE_50 = 50,
  RAYTRACE_EEVEE_RESOLUTION_SCALE_25 = 25,
};
...
enum RaytraceEEVEE_Method {
  ...
  /* Experimental backend slot for future scene-intersection tracing. */
  RAYTRACE_EEVEE_METHOD_HARDWARE = 2,
};

enum RaytraceEEVEE_SpecularMode {
  RAYTRACE_EEVEE_SPECULAR_MODE_OFF = 0,
  RAYTRACE_EEVEE_SPECULAR_MODE_HYBRID = 1,
  RAYTRACE_EEVEE_SPECULAR_MODE_FULL_RT = 2,
  RAYTRACE_EEVEE_SPECULAR_MODE_AUTO = 3,
};

enum RaytraceEEVEE_GIMode {
  RAYTRACE_EEVEE_GI_MODE_ACCURATE = 0,
  /* Deprecated compatibility values now versioned/sanitized to ACCURATE. */
  RAYTRACE_EEVEE_GI_MODE_REFINE = 1,
  RAYTRACE_EEVEE_GI_MODE_OFF = 2,
  RAYTRACE_EEVEE_GI_MODE_FAST = 3,
};

enum SceneEEVEE_HardwareIndirectFlag {
  SCE_EEVEE_HWRT_FAST_GI = (1 << 0),
  SCE_EEVEE_HWRT_CAUSTICS = (1 << 1),
};
...
struct RaytraceEEVEE {
  ...
  /** #RaytraceEEVEE_ResolutionScale. Percentage of render resolution used by the ray grid. */
  int resolution_scale = RAYTRACE_EEVEE_RESOLUTION_SCALE_50;
  /** #RaytraceEEVEE_Flag. */
  ...
  /** #RaytraceEEVEE_DenoiseFilter. */
  int denoise_filter = RAYTRACE_EEVEE_DENOISE_FILTER_OIDN;
  /** #RaytraceEEVEE_DenoiseQuality. */
  int denoise_quality = RAYTRACE_EEVEE_DENOISE_QUALITY_BALANCED;
  /** #RaytraceEEVEE_DenoiseInputPasses. */
  int denoise_input_passes = RAYTRACE_EEVEE_DENOISE_INPUT_RGB_ALBEDO_NORMAL;
  /** #RaytraceEEVEE_DenoisePrefilter. */
  int denoise_prefilter = RAYTRACE_EEVEE_DENOISE_PREFILTER_FAST;
  /** Use the GPU OIDN backend. CPU OIDN is not wired for Nuru HWRT yet. */
  int denoise_use_gpu = 1;
  /** Run OIDN every N samples; spatial/reconstruction continues every sample. */
...
struct SceneEEVEE {
  ...
  int flag = SCE_EEVEE_TAA_REPROJECTION | SCE_EEVEE_SHADOW_ENABLED | SCE_EEVEE_SSR_ENABLED;
  ...
  float indirect_light_intensity = 5.0f;

  int ray_tracing_method = RAYTRACE_EEVEE_METHOD_HARDWARE;
  int ray_tracing_gi_bounces = 1;
  int ray_tracing_reflection_bounces = 3;
  int ray_tracing_refraction_bounces = 3;
  int _pad_ray_tracing_temporal_samples[2] = {0, 0};
  /** Per-feature Hardware RT overrides. Disabled features keep classic Eevee behavior. */
  int hardware_raytracing_features = RAYTRACE_EEVEE_HARDWARE_GI |
                                    ...
                                    RAYTRACE_EEVEE_HARDWARE_SECONDARY_GI;
  ...
  /** Deprecated compatibility storage for removed secondary GI. */
  int use_hardware_raytracing_indirect_gi_cache = 0;
  /** Deprecated compatibility storage for removed secondary GI resolution. */
  int hardware_raytracing_indirect_gi_resolution = 4;
  /** Reflection ownership mode for the experimental Hardware RT path. */
  int hardware_raytracing_reflection_mode = RAYTRACE_EEVEE_SPECULAR_MODE_FULL_RT;
  /** Refraction ownership mode for the experimental Hardware RT path. */
  int hardware_raytracing_refraction_mode = RAYTRACE_EEVEE_SPECULAR_MODE_FULL_RT;
  int ray_tracing_caustics_samples = 8;
  /** Kernel samples per Hardware RT shadow visibility evaluation. */
  int hardware_raytracing_shadow_samples = 2;
  /** Nuru: saturation of tinted transmission shadows under refractive materials. */
  float hardware_raytracing_shadow_color_intensity = 0.5f;
  /** Nuru: multiplier on the sharp caustic peak inside the transparent-shadow trace. 1.0 is
   * the natural intensity; raise (up to 10) for dramatic concentrated highlights, lower
   * (down to 0.5) for a subtler caustic. Independent of the Color Intensity slider so the
   * tinted shadow component is unaffected. */
  float hardware_raytracing_shadow_photons_intensity = 10.0f;
  char hardware_raytracing_indirect_flags = 0;
  char _pad_ray_tracing[3] = {};
  /** Nuru: blend HWRT shadow between opaque (0) and transparent transmission trace (1).
   * Reuses the deprecated environment-shadow-softness slot to preserve DNA size/alignment. */
  float hardware_raytracing_shadow_transparency = 0.5f;
  int _pad_ray_tracing_options = 0;
