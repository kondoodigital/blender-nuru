...
class ObjectRef {
 ...
 public:
  ...
  /* Dupli draw iteration can wrap the source object in a temporary shallow copy. */
  Object *source_object() const
  {
    return (dupli_object_ && dupli_object_->ob) ? dupli_object_->ob : object;
  }

  /* Sparse HWRT hit-eval replay must not hold onto temporary converted wrapper objects. */
  Object *stable_hit_eval_object() const
  {
    Object *source = source_object();
    if (source == nullptr || source->type != OB_MESH || source->data == nullptr ||
        GS(static_cast<ID *>(source->data)->name) != ID_ME)
    {
      return nullptr;
    }
    if (!dupli_object_) {
    ...
    if (object->type != source->type || object->data != source->data) {
      return nullptr;
    }
    return source;
  }
