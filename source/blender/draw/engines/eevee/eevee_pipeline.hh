...
struct DeferredLayerBase {
  ...
  void gbuffer_pass_sync(Instance &inst,
                         DRWState depth_state = DRW_STATE_DEPTH_EQUAL,
...
class DeferredLayer : DeferredLayerBase {
 ...
 private:
  ...
  gpu::Texture *shared_indirect_tx_ = nullptr;
  ...
  bool use_hardware_rt_shadows_ = false;
  bool use_hardware_rt_environment_visibility_ = false;
  bool defer_scene_final_hardware_specular_ = false;
  bool use_clamp_direct_ = false;
  bool use_clamp_indirect_ = false;

  /* Post-combine pass-export compute. Rewrites the COLOR-storage viewport render passes
   * (Diffuse Color/Light, Specular Color/Light, Normal, Position) into `rp_color_tx` from
   * compute, because the combine fragment shader's `output_renderpass_color` writes do not
   * reliably reach `rp_color_tx` under Nuru on Metal. Dispatch is gated by
   * `any_pass_export_enabled_` so a Combined-only viewport pays nothing. */
  PassSimple pass_export_ps_ = {"Combine.PassExport"};
  /* Referenced by `pass_export_ps_`'s dispatch; updated per `render()` call. */
  int3 pass_export_dispatch_size_ = int3(1);
  bool any_pass_export_enabled_ = false;
 ...
 public:
  ...
  void begin_sync(DRWState gbuffer_depth_state = DRW_STATE_DEPTH_EQUAL,
                  bool gbuffer_write_depth = false);
...
class DeferredPipeline {
 ...
 private:
  ...
  DeferredLayer thin_glass_layer_;
  Framebuffer thin_glass_overlay_fb_ = {"thin_glass_overlay_fb"};
  TextureFromPool thin_glass_overlay_radiance_tx_ = {"thin_glass_overlay_radiance_tx"};

  PassSimple debug_draw_ps_ = {"debug_gbuffer"};

 public:
  DeferredPipeline(Instance &inst)
      : opaque_layer_(inst), refraction_layer_(inst), thin_glass_layer_(inst) {};
  ...
  void render_thin_glass_overlay(View &render_view, int2 extent);

  /* Return the maximum amount of gbuffer layer needed. */
  int header_layer_count() const
  {
    return max_iii(opaque_layer_.header_layer_count(),
                   refraction_layer_.header_layer_count(),
                   thin_glass_layer_.header_layer_count());
  }

  /* Return the maximum amount of gbuffer layer needed. */
  int closure_layer_count() const
  {
    return max_iii(opaque_layer_.closure_layer_count(),
                   refraction_layer_.closure_layer_count(),
                   thin_glass_layer_.closure_layer_count());
  }

  /* Return the maximum amount of gbuffer layer needed. */
  int normal_layer_count() const
  {
    return max_iii(opaque_layer_.normal_layer_count(),
                   refraction_layer_.normal_layer_count(),
                   thin_glass_layer_.normal_layer_count());
  }
  ...
  bool is_empty() const
  {
    return opaque_layer_.is_empty() && refraction_layer_.is_empty() && thin_glass_layer_.is_empty();
  }

  eClosureBits closure_bits_get() const
  {
    return opaque_layer_.closure_bits_get() | refraction_layer_.closure_bits_get() |
           thin_glass_layer_.closure_bits_get();
  }
