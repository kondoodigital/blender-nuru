...
static bool material_contains_bsdf_transparent(const blender::Material *blender_mat)
{
  ...
  for (const bNode *node = static_cast<const bNode *>(blender_mat->nodetree->nodes.first);
       ...
       node = node->next)
  {
    if (node->type_legacy == SH_NODE_BSDF_TRANSPARENT) {
      return true;
    }
  }
  return false;
...
static int material_surface_render_method_get(const Instance &inst,
                                              const blender::Material *blender_mat)
{
  if (inst.raytracing.use_hardware_tracing_method()) {
    return MA_SURFACE_METHOD_DEFERRED;
  }
  return blender_mat->surface_render_method;
}
                                               ...
                                               eMaterialProbe probe_capture)
{
   ...
   * Hit-eval materials also need to be ready immediately: falling back to the default material for
   * traced secondary hits leaves reflections/refractions with flat proxy shading until a later
   * sync, which is especially visible on procedural materials. */
                                        ...
                                        bool has_motion)
{
  ...
  const bool use_forward_pipeline = (material_surface_render_method_get(inst_, blender_mat) ==
                                    ...
                                    !material_contains_bsdf_transparent(blender_mat);
