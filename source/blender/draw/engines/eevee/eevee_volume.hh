...
class VolumeModule {
 ...
 public:
  ...
  {
    /* Nuru: Eevee volumetrics run on the Vulkan and Metal backends. Both keep the retained
     * integration-shader bounds clamp (invariant A:
     * `min(uniform_buf.volumes.tex_size, imageSize(out_scattering_img))`) as the active store
     * guard, and the vanilla pass order required by invariant Q (`volume.draw_prepass` ->
     * `deferred.render`, `volume.draw_compute` after `background.render`, `volume.draw_resolve`
     * before `forward.render`) is unchanged in `eevee_view.cc`. */
    /* Metal re-enabled on June 11 2026 after the root cause of the 2026-05-21 Apple Silicon
     * full-system wedge was closed by two independent fixes: the UniformData UBO layout shift
     * that fed garbage `volumes.tex_size` to the integration loop (`eevee_raytrace_shared.hh`,
     * DIAMOND 3) and the unbounded vanilla `i <= tex_size.z` loop (invariant-A clamp restored
     * in `eevee_volume_integration_comp.glsl`, DIAMOND 10). User-validated on Apple Silicon
     * under the historical kill conditions: object scatter/absorption fog, world volume, lit
     * fog with shadowed sun shafts, 25+ minutes of interactive rendered-viewport load and F12
     * renders with zero wedge signatures (see docs/agent_handoff.md, June 11 2026). */
