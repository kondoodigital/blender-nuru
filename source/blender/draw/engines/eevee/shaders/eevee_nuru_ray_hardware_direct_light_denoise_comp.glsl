...
{
  LightData light = light_buf[local_light_index];
  light.color = abs(light.color);
  return light;
}
                                             ...
                                             uchar receiver_light_set)
{
  ...
  if ((cl.type == CLOSURE_BSDF_MICROFACET_GGX_REFLECTION_ID) ||
      (cl.type == CLOSURE_BSDF_MICROFACET_GGX_REFRACTION_ID))
  {
    /* Glass caustics live in the shadow atlas as additive peaks; specular closures must not
     * treat visibility > 1 as extra direct light on metal or glass. */
    ...
    for (uint x = 0u; x < tile_size_px; x++) {
      ...
      if (use_exact_local_lights) {
        for (uint local_light_index = 0u;
             ...
             local_light_index++)
        {
          const LightData light = light_buf[local_light_index];
          const float3 local_visibility = texelFetch(
                                              hardware_rt_shadow_visibility_tx,
                                              int3(texel, light.shadow_layer),
                                              0)
                                              .rgb;
        ...
        for (uint sample_index = 0u; sample_index < samples_per_point; sample_index++) {
          ...
          /* Nuru: `local_light_index` is into the culled+sorted `light_buf`. Translate to the
           * stable visibility layer through `light.shadow_layer`. RGB carries the transparent
           * shadow attenuation. */
          const LightData picked_light = light_buf[local_light_index];
          const float3 local_visibility = texelFetch(hardware_rt_shadow_visibility_tx,
                                                     int3(texel, picked_light.shadow_layer),
                                                     0)
                                              .rgb;
      ...
      if (total_sun_importance > 0.0f) {
        for (uint sample_index = 0u; sample_index < samples_per_point; sample_index++) {
          ...
          const uint sun_light_index = uniform_buf.raytrace.hardware_direct_light.local_lights_len +
                                       sun_index;
          const LightData sun_light = light_buf[sun_light_index];
          const float3 sun_visibility = texelFetch(hardware_rt_shadow_visibility_tx,
                                                   int3(texel, sun_light.shadow_layer),
                                                   0)
                                            .rgb;
