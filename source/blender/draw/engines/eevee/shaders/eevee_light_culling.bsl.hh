...
struct Cull {
  ...
  [[storage(5, read_write)]] LightData (&out_world_sun_buf)[2];
  [[storage(6, read_write)]] float4 (&out_world_sun_direction_buf)[2];
};
               ...
               [[local_invocation_index]] const uint local_index)
{
  ...
  if (is_sun_light(light.type)) {
    ...
    if (is_world_sun_light) {
      ...
      float3 world_sun_override_direction = transform_z_axis(srt.out_world_sun_buf[l_idx].object_to_world);
      bool use_world_sun_override = dot(world_sun_override_direction, world_sun_override_direction) >
                                    1.0e-8f;
      if (use_world_sun_override) {
        world_sun_override_direction = normalize(world_sun_override_direction);
        light.object_to_world.x.z = world_sun_override_direction.x;
        light.object_to_world.y.z = world_sun_override_direction.y;
        light.object_to_world.z.z = world_sun_override_direction.z;
      }

      LightSunData sun_data = light.sun();
      sun_data.direction = use_world_sun_override ? world_sun_override_direction :
                                                   transform_z_axis(light.object_to_world);
      light.sun() = sun_data;
      srt.out_world_sun_buf[l_idx] = light;
      const float direction_len_sq = dot(sun_data.direction, sun_data.direction);
      srt.out_world_sun_direction_buf[l_idx] = (direction_len_sq > 1.0e-8f) ?
                                                   float4(normalize(sun_data.direction), 1.0f) :
                                                   float4(0.0f);
