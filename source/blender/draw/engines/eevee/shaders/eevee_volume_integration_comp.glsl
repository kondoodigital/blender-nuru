...
void main()
{
  ...
  /* Nuru retained crash fix (invariant A, bounds and store safety): bound the integration by
   * both the host-pushed froxel size and the real allocated image extent, and loop with
   * `i < volume_size.z`. Vanilla loops `i <= tex_size.z`, making the last iteration read and
   * store one slice past the allocation (`AllocSize == tex_size`); the unchecked
   * `imageStoreFast` then writes out of bounds on every froxel column. This wedged the Metal
   * `Volume.Integration` pass on Apple Silicon and the Vulkan integration dispatch on NVIDIA
   * 595.71 (Xid 109 CTX SWITCH TIMEOUT, June 11 2026 re-enable validation).
   * Store-bound proof: `froxel = (texel, i)` with `texel < volume_size.xy` (early-out below)
   * and `i < volume_size.z`, where `volume_size <= imageSize(out_scattering_img)` per the min
   * below, so every store satisfies `coord_i < AllocSize(tex)_i` for all dimensions. */
  int3 volume_size = min(uniform_buf.volumes.tex_size, int3(imageSize(out_scattering_img)));

  if (any(greaterThanEqual(texel, volume_size.xy))) {
    return;
  }
