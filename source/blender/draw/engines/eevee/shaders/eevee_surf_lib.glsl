...
#if defined(GPU_FRAGMENT_SHADER) && !defined(MAT_SHADOW)
void nodetree_surface_material_filter_eval(float closure_rand)
{
  /* Single sharp evaluation. Roughness must not widen base-color or other albedo texture
   * lookups; specular softness comes from the BSDF and lighting paths, not from blurring
   * material inputs during surface evaluation. */
  closure_evaluation_reset(closure_rand);
