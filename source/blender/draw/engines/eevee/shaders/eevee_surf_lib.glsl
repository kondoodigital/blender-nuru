...
{
  material_texture_filter_disable();
  closure_evaluation_reset(closure_rand);
  nodetree_surface(closure_rand);

  const float filter_roughness = material_texture_filter_capture_roughness();
  if (!(filter_roughness > 1.0e-3f)) {
    return;
  }

  closure_evaluation_reset(closure_rand);
  material_texture_filter_enable(filter_roughness);
  nodetree_surface(closure_rand);
  material_texture_filter_disable();
}
#endif
