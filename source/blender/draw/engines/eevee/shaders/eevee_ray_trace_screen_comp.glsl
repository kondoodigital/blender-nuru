     ...
     * direction over many rays. */
               ...
               !screen_hit_closure_is_diffuse_gi(closure_type) && lightprobe_uses_world(samp))
      {
        ...
        if (use_hardware_rt_environment_visibility && lightprobe_uses_world(samp)) {
           ...
           * reflections through that bent-dome signal, but still attenuate them by the traced
           * receiver visibility: an unmasked specular world fallback floods enclosed interiors
           * with sky/HDRI light (the dome visibility is 0 there). Use the cross-filtered load:
           * the per-texel value is few-sample quantized and flickers temporally. */
          HardwareEnvironmentVisibilityData env_visibility =
              hardware_environment_visibility_load_filtered(texel_fullres, Ng);
          if (screen_hit_closure_is_diffuse_gi(closure_type)) {
