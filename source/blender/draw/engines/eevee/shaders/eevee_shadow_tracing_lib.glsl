...
#if defined(SHADOW_DISPATCH_USE_GLOBAL_TEXEL)
/* Nuru: Dispatch texel filled by the calling shader before light/shadow evaluation. Declared here
 * (not in the calling shader) so the declaration precedes its use below once the dependency-ordered
 * GLSL is composed for backends that consume it directly (Vulkan). */
                  ...
                  int ray_step_count)
{
  ...
  if (is_transmission && !is_facing_light) {
    ...
    P += abs(is_directional ? abs(thickness) :
                              min(abs(thickness), distance_to_shadow - 0.01f)) *
         L;
  }
...
bool light_is_world_sun(uint l_idx)
{
  if (l_idx < light_cull_buf.local_lights_len) {
    return false;
  }
  const uint sun_index = l_idx - light_cull_buf.local_lights_len;
  return sun_index < uniform_buf.raytrace.hardware_direct_light.world_sun_lights_len;
}

float3 shadow_eval_dispatch(uint l_idx,
                            LightData light,
                            const bool is_directional,
                            const bool is_transmission,
                            bool is_translucent_with_thickness,
...
{
#if defined(SHADOW_DISPATCH_USE_GLOBAL_TEXEL)
  const int2 shadow_dispatch_texel = shadow_dispatch_texel_fullres;
#else
  const int2 shadow_dispatch_texel = int2(gl_FragCoord.xy);
#endif

#if defined(SHADOW_DISPATCH_USE_GLOBAL_HARDWARE_RT)
  const bool shadow_dispatch_can_use_hardware_rt = shadow_dispatch_use_hardware_rt;
#else
  ...
  /* Nuru: HWRT shadow visibility is RGB so refractive shadows carry per-channel tint. The VSM
   * path stays naturally monochrome and broadcasts its scalar into the float3 return below. */
...
#    define SHADOW_DISPATCH_HARDWARE_VISIBILITY_FETCH(_texel, _layer) \
      texelFetch(hardware_rt_shadow_visibility_tx, int3((_texel), (_layer)), 0).rgb
#  endif
...
#  endif
  ...
  /* Nuru: HWRT shadow availability is driven by `cast_shadow`, not the VSM tilemap. VSM is
   * intentionally skipped for HWRT-covered lights to keep the shadow page pool from overflowing
   * (see `ShadowModule::end_sync`). */
  const bool use_direct_hardware_rt = use_hardware_rt_shadows && light.cast_shadow;
  if (shadow_dispatch_can_use_hardware_rt &&
      (use_direct_hardware_rt || use_world_hardware_rt) &&
      (!is_transmission || SHADOW_DISPATCH_ALLOW_TRANSMISSION_HARDWARE_RT) &&
      !is_translucent_with_thickness)
  {
    /* Nuru: `l_idx` is the culled+Z-sorted light index and changes with camera rotation, but
     * the visibility texture is filled in unsorted per-type order. `light.shadow_layer` is
     * assigned once at sync time so the layer mapping is stable across rotations. */
    const int shadow_layer = light.shadow_layer;
    return SHADOW_DISPATCH_HARDWARE_VISIBILITY_FETCH(shadow_dispatch_texel, shadow_layer);
  }
#endif

  return float3(shadow_eval(light,
                            is_directional,
                            is_transmission,
                            is_translucent_with_thickness,
                            thickness,
