...
VERTEX_SOURCE("eevee_nuru_geom_hit_fullscreen_vert.glsl")
VERTEX_OUT(eevee_surf_iface)
...
BUILTINS(BuiltinBits::VERTEX_ID)
VERTEX_SOURCE("eevee_nuru_geom_hit_mesh_vert.glsl")
VERTEX_OUT(eevee_surf_iface)
...
PUSH_CONSTANT(int2, gpu_attr_1)
PUSH_CONSTANT(int, gpu_attr_1_meta)
/* Element offset of the per-material index-buffer subrange. The BLAS is built with this offset
 * applied, so hit primitive ids are subrange-relative while the SSBO bind exposes the whole
 * buffer. Multi-material meshes replay the wrong triangles (stretched textures) without it. */
PUSH_CONSTANT(int, hit_eval_index_start)
GPU_SHADER_CREATE_END()
