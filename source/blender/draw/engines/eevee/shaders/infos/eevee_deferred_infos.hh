...
SAMPLER(10, sampler2D, hardware_rt_environment_visibility_tx)
...
SPECIALIZATION_CONSTANT(bool, use_hardware_direct_light, false)
SPECIALIZATION_CONSTANT(bool, defer_hardware_specular_indirect, false)
...
SAMPLER(9, sampler2D, hardware_rt_caustics_tx)
SAMPLER(12, sampler2D, hardware_direct_light_tx)
SAMPLER(13, sampler2DDepth, depth_tx)
SAMPLER(14, sampler2D, shared_indirect_tx)
IMAGE(5, SFLOAT_16_16_16_16, read_write, image2D, radiance_feedback_img)
FRAGMENT_OUT_DUAL(0, float4, out_combined_add, SRC_0)
FRAGMENT_OUT_DUAL(0, float4, out_combined_mul, SRC_1)
TYPEDEF_SOURCE("eevee_defines.hh")
ADDITIONAL_INFO(eevee_gbuffer_data)
ADDITIONAL_INFO(eevee_render_pass_out)
ADDITIONAL_INFO(eevee_hiz_data)
...
/* -------------------------------------------------------------------- */
/** \name Deferred Pass Export
 *
 * Compute pass run after `eevee_deferred_combine` that re-writes the EEVEE COLOR-storage
 * viewport render passes (Normal, Position, Diffuse Color/Light, Specular Color/Light)
 * into `rp_color_tx`. Under the Nuru pipeline on Metal the per-pass `output_renderpass_color`
 * writes inside the combine fragment shader do not reliably reach `rp_color_tx`, leaving
 * those pass displays gray; this compute pass recomputes the same values from the closure
 * gbuffer and direct/indirect radiance and writes them via `imageStore`, which does reach.
 *
 * The dispatch is gated on the C++ side by `DeferredLayer::any_pass_export_enabled_` so
 * Combined-only viewports pay nothing; within the shader the per-pass writes are gated by
 * specialization-constant pass IDs (`-1` when the pass is not requested) so unused slots
 * compile away.
 * \{ */

GPU_SHADER_CREATE_INFO(eevee_deferred_pass_export)
DO_STATIC_COMPILATION()
...
SAMPLER(7, sampler2D, indirect_radiance_2_tx)
SAMPLER(8, sampler2D, indirect_radiance_3_tx)
SAMPLER(9, sampler2D, shared_indirect_tx)
COMPUTE_SOURCE("eevee_nuru_deferred_pass_export_comp.glsl")
SPECIALIZATION_CONSTANT(bool, use_split_radiance, true)
SPECIALIZATION_CONSTANT(bool, use_shared_indirect, false)
SPECIALIZATION_CONSTANT(bool, defer_hardware_specular_indirect, false)
SPECIALIZATION_CONSTANT(int, pass_diffuse_color_id, -1)
SPECIALIZATION_CONSTANT(int, pass_diffuse_light_id, -1)
SPECIALIZATION_CONSTANT(int, pass_specular_color_id, -1)
SPECIALIZATION_CONSTANT(int, pass_specular_light_id, -1)
SPECIALIZATION_CONSTANT(int, pass_normal_id, -1)
SPECIALIZATION_CONSTANT(int, pass_position_id, -1)
GPU_SHADER_CREATE_END()
