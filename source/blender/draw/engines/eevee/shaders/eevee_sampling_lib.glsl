...
/* Pick a representative full-resolution texel inside a reduced-resolution trace block.
 * The default global bias can make GI undersampling line up into broad screen-space structures;
 * add a per-block low-discrepancy phase so neighboring blocks cover different sub-texels while
 * still changing over time. */
int2 raytrace_ceil_divide(int2 numerator, int denominator)
{
  return (numerator + int2(max(denominator, 1) - 1)) / max(denominator, 1);
}

int2 raytrace_texel_to_fullres(int2 texel,
                               int resolution_scale,
                               int resolution_scale_denominator,
                               int2 resolution_bias)
{
  int scale = max(resolution_scale, 1);
  int denominator = max(resolution_scale_denominator, 1);
  int2 cell_min = raytrace_ceil_divide(texel * scale, denominator);
  int2 cell_max = raytrace_ceil_divide((texel + int2(1)) * scale, denominator) - int2(1);
  int2 cell_extent = max(cell_max - cell_min + int2(1), int2(1));
  int2 local_offset = min((max(resolution_bias, int2(0)) * cell_extent) / scale,
                          cell_extent - int2(1));
  return cell_min + local_offset;
}

int2 raytrace_fullres_to_texel(int2 texel_fullres,
                               int resolution_scale,
                               int resolution_scale_denominator,
                               int2 resolution_bias)
{
  return max(int2(0), texel_fullres * max(resolution_scale_denominator, 1)) /
         max(resolution_scale, 1);
}

int2 raytrace_representative_fullres_texel(int2 texel,
                                           int resolution_scale,
                                           int resolution_scale_denominator,
                                           int2 resolution_bias)
{
  ...
  float2 phase = (float2(resolution_bias) + 0.5f) / float(resolution_scale);
  float2 block_noise = interleaved_gradient_noise(
      float2(texel) + 0.5f, float2(19.0f, 47.0f), phase);
  int scale = max(resolution_scale, 1);
  int denominator = max(resolution_scale_denominator, 1);
  int2 cell_min = raytrace_ceil_divide(texel * scale, denominator);
  int2 cell_max = raytrace_ceil_divide((texel + int2(1)) * scale, denominator) - int2(1);
  int2 cell_extent = max(cell_max - cell_min + int2(1), int2(1));
  int2 local_offset = min(int2(floor(block_noise * float2(cell_extent))), cell_extent - int2(1));
  return cell_min + local_offset;
}

int2 raytrace_representative_fullres_texel(int2 texel, int resolution_scale, int2 resolution_bias)
{
  return raytrace_representative_fullres_texel(texel, resolution_scale, 1, resolution_bias);
}
