...
struct DenoiseSpatial {
  ...
  [[image(6, read, SFLOAT_16_16_16_16)]] image2D hit_position_img;

  [[image(7, read, RAYTRACE_TILEMASK_FORMAT)]] uimage2DArray tile_mask_img;
                  ...
                  [[local_invocation_id]] const uint3 local_id)
{
  ...
  float3 closest_hit_direction = float3(0.0f, 0.0f, 1.0f);
  ...
  for (uint i = 0u; i < sample_count; i++) {
    ...
    if (ray_time > 0.0f && ray_time < closest_hit_time) {
      closest_hit_time = ray_time;
      closest_hit_direction = ray_direction;
    }
  ...
  float2 hit_uv = (float2(texel_fullres) + 0.5f) * uniform_buf.raytrace.full_resolution_inv;
  float hit_depth = depth;
  if (closest_hit_time < 1.0e9f) {
    float3 P_hit = imageLoad(srt.hit_position_img, texel).xyz;
    if (dot(P_hit, P_hit) == 0.0f) {
      float3 surface_P = drw_point_screen_to_world(float3(hit_uv, depth));
      P_hit = surface_P + closest_hit_direction * closest_hit_time;
    }
    hit_depth = project_point(drw_view().winmat, drw_point_world_to_view(P_hit)).z * 0.5f + 0.5f;
  }
