...
/**
 * Return cached light-probe data at P using an explicit noise pixel (full-resolution
 * coordinates when ray tracing runs at reduced dispatch resolution).
 */
LightProbeSample lightprobe_sample(float2 noise_pixel, float3 P, float3 Ng, float3 V)
{
  float noise = interleaved_gradient_noise(noise_pixel, 0.0f, 0.0f);
...
{
  return lightprobe_sample(UTIL_TEXEL, P, Ng, V);
}
...
bool lightprobe_uses_world(LightProbeSample samp)
{
  return lightprobe_sphere_buf[samp.spherical_id].influence_scale == 0.0f;
}
    ...
    LightProbeSample samp, ClosureSubsurface cl, float3 /*P*/, float3 /*V*/, float thickness)
{
  ...
  float3 radiance_sh = spherical_harmonics_evaluate_lambert(cl.N, samp.volume_irradiance).rgb;
  radiance_sh += spherical_harmonics_evaluate_lambert(-cl.N, samp.volume_irradiance).rgb * sss_profile;
  return radiance_sh;
}
    ...
    LightProbeSample samp, ClosureUndetermined cl, float3 P, float3 V, float thickness)
{
  ...
  float3 radiance_sh = spherical_harmonics_evaluate_lambert(ray.dominant_direction, samp.volume_irradiance).rgb;
  return mix(radiance_cube, radiance_sh, fac);
}

float3 lightprobe_eval_with_direction(LightProbeSample samp,
                                      ...
                                      float3 direction)
{
  ...
  float3 radiance_cube = lightprobe_spherical_sample_normalized_with_parallax(
      samp, P, direction, lod);
  float3 radiance_sh = spherical_harmonics_evaluate_lambert(direction, samp.volume_irradiance).rgb;
  return mix(radiance_cube, radiance_sh, fac);
}
