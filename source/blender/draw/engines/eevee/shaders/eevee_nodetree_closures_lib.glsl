...
/* Thin Glass pass-through controls written by the Thin Glass BSDF node. */
packed_float3 g_thin_glass_color;
float g_thin_glass_film_alpha;
float g_thin_glass_reflection_weight;
...
void closure_weights_reset(float closure_rand)
{
  ...
  g_thin_glass_color = float3(1.0f);
  g_thin_glass_film_alpha = 0.0f;
  g_thin_glass_reflection_weight = 1.0f;
}
