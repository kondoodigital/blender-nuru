...
void light_eval_single_closure(
    LightData light, LightVector lv, ClosureLight &cl, float3 V, float attenuation, float3 shadow)
{
  /* Nuru "Visible in Reflections": specular lobes paint the light's image (sharp mirror or
   * blurry rough-metal blob alike). The per-light toggle (default off) suppresses this light's
   * contribution to ALL specular lobes; diffuse lighting is untouched. */
  if (cl.type == LIGHT_SPECULAR && light.visible_in_reflections == false) {
    return;
  }
  /* Nuru: `shadow` is RGB so transparent shadows can tint the light per channel (e.g. green
   * glass darkens red/blue more than green). The VSM path broadcasts its scalar to float3 so
   * this signature stays uniform across HWRT / non-HWRT. */
  ...
  float3 visibility = shadow * attenuation;
                       ...
                       float terminator_geometry_offset)
{
  ...
  /* Nuru: VSM is intentionally skipped for HWRT-covered lights; use `cast_shadow` so HWRT still
   * fires when there is no VSM tilemap. */
  const bool use_direct_hardware_rt_shadow = use_hardware_rt_shadows && light.cast_shadow;
  evaluate_shadow = evaluate_shadow || use_direct_hardware_rt_shadow ||
                    (use_hardware_rt_environment_visibility && light_is_world_sun(l_idx));
#endif
  if (evaluate_shadow) {
    shadow = shadow_eval_dispatch(l_idx,
                                  light,
                                  is_directional,
                                  is_transmission,
