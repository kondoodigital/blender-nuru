...
void main()
{
  ...
  if (is_ray_tracing) {
    int denoise_tile_span = max(
        1, (resolution_scale + max(resolution_scale_denominator, 1) - 1) /
               max(resolution_scale_denominator, 1));
    int2 tile_base = (tile * resolution_scale) / max(resolution_scale_denominator, 1);
    for (int x_tile = 0; x_tile < denoise_tile_span; x_tile++) {
      for (int y_tile = 0; y_tile < denoise_tile_span; y_tile++) {
        int2 tile_adj = tile_base + int2(x_tile, y_tile);
