...
#elif (CLOSURE_BIN_COUNT == 3) && defined(MAT_REFRACTION) && defined(MAT_REFLECTION) && \
    !defined(MAT_CLEARCOAT)
  /* Hit-eval replay can request three bins for mixed Principled materials so the base diffuse lobe
   * no longer competes with refraction for the same slot. That avoids a hard replay flip once
   * Transmission Weight overtakes the remaining diffuse contribution. */
  closure_select(g_closure_bins[1], g_closure_rand[1], cl);
#else
...
Closure closure_eval(ClosureReflection reflection)
{
  ...
  cl.data.g = g_thin_glass_film_alpha;
  cl.data.b = g_thin_glass_reflection_weight;
#endif
...
/* Thin Glass BSDF. */
Closure closure_eval(ClosureReflection reflection, ClosureTransparency transparency)
{
  closure_eval(reflection);
  closure_eval(transparency);
  return Closure(0);
}
...
bool closure_type_uses_material_texture_filter(ClosureType type)
{
  return (type == CLOSURE_BSDF_MICROFACET_GGX_REFLECTION_ID) ||
         (type == CLOSURE_BSDF_MICROFACET_GGX_REFRACTION_ID);
}

void closure_evaluation_reset(float closure_rand)
{
  closure_weights_reset(closure_rand);
  g_closure_reflection_bin = true;
}

float material_texture_filter_capture_roughness()
{
  float roughness = 0.0f;
  for (int i = 0; i < CLOSURE_BIN_COUNT; i++) {
    ClosureUndetermined cl = g_closure_get(uchar(i));
    if (!closure_type_uses_material_texture_filter(cl.type) || (cl.weight <= 1.0e-5f)) {
      continue;
    }
    if ((cl.type == CLOSURE_BSDF_MICROFACET_GGX_REFRACTION_ID) &&
        (abs(cl.data.y - 50.0f) <= 1.0e-4f))
    {
      /* Nuru Translucent BSDF: keep the hidden rough refraction closure for light transport, but
       * do not let that roughness prefilter the material texture. The color texture is the sharp
       * visibility tint multiplied onto the rough closure. */
      continue;
    }
    roughness = max(roughness, saturate(cl.data.x));
  }
  return roughness;
}

#define EEVEE_MATERIAL_TEXTURE_FILTER

float g_material_texture_filter_roughness = 0.0f;

void material_texture_filter_disable()
{
  g_material_texture_filter_roughness = 0.0f;
}

void material_texture_filter_enable(float roughness)
{
  g_material_texture_filter_roughness = saturate(roughness);
}

bool material_texture_filter_active()
{
#ifdef GPU_FRAGMENT_SHADER
  return g_material_texture_filter_roughness > 1.0e-3f;
#else
  return false;
#endif
}

float material_texture_filter_blend_get()
{
  float roughness = saturate(g_material_texture_filter_roughness);
  return roughness * roughness;
}

float material_texture_filter_procedural_width_get()
{
  /* Shared width hint for procedural texture nodes that implement roughness filtering. */
  return material_texture_filter_blend_get();
}

float material_texture_filter_image_lod_scale_get()
{
  /* Image textures can use their existing mip chain instead of remapping colors. Keep low
   * roughness close to the original footprint and widen only as the material gets visibly rough. */
  return mix(1.0f, 64.0f, material_texture_filter_blend_get());
}
...
#if defined(DRW_CUSTOM_WORLD_INCIDENT_VECTOR) && !defined(GPU_METAL)
/* Nuru: Hook implemented by the shader main file (e.g. hit-eval geometry). Forward declared for
 * the plain GLSL path which compiles dependencies before the main file. Metal must not see this
 * prototype: the MSL generator already forward-declares every function as a shader-class member,
 * so an explicit prototype becomes a duplicate member declaration and fails compilation. */
...
#ifdef MAT_GEOM_WORLD
  return N;
#elif defined(DRW_CUSTOM_WORLD_INCIDENT_VECTOR)
  return -reflect(drw_custom_world_incident_vector(P), N);
#else
...
float3 coordinate_incoming(float3 P)
{
  ...
  return -P;
#elif defined(DRW_CUSTOM_WORLD_INCIDENT_VECTOR)
  return drw_custom_world_incident_vector(P);
#else
...
float texture_lod_bias_base_get()
{
...
{
  float lod_bias = texture_lod_bias_base_get();
#ifdef GPU_FRAGMENT_SHADER
  if (material_texture_filter_active()) {
    lod_bias *= material_texture_filter_image_lod_scale_get();
  }
#endif
  return lod_bias;
}
