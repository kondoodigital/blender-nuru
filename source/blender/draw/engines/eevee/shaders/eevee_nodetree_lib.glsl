...
float material_texture_filter_checker_width_get()
{
  /* Match the GGX alpha convention before converting to a procedural-cell footprint. This is a
   * conservative material-filter proxy; roughness is angular, so there is no scene-independent
   * physically exact mapping to checker cell width. */
