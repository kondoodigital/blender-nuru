...
void main()
{
  ...
  if (use_transmission) {
    ...
    if (cl_transmit.type == CLOSURE_BSSRDF_BURLEY_ID) {
      ...
      float3 sss_profile = subsurface_transmission(to_closure_subsurface(cl_transmit).sss_radius, abs(thickness));
  ...
  if (use_lightprobe_eval) {
    ...
    bool use_environment_visibility = use_hardware_rt_environment_visibility &&
                                      lightprobe_uses_world(samp);
    HardwareEnvironmentVisibilityData env_visibility;
    if (use_environment_visibility) {
      env_visibility = hardware_environment_visibility_load(texel, Ng);
      use_environment_visibility = hardware_environment_visibility_is_valid(env_visibility);
    }
    for (uchar i = 0; i < LIGHT_CLOSURE_EVAL_COUNT && i < closure_count; i++) {
      ClosureUndetermined cl = gbuf.layer_get(i);
      const bool specular_closure = (cl.type == CLOSURE_BSDF_MICROFACET_GGX_REFLECTION_ID) ||
                                    (cl.type == CLOSURE_BSDF_MICROFACET_GGX_REFRACTION_ID);
      float3 indirect_light = lightprobe_eval(samp, cl, P, V, thickness);
      if (defer_hardware_specular_indirect && specular_closure) {
        /* HWRT owns bulk specular indirect, but Principled dielectric still needs the
         * grazing-angle Fresnel layer on top of diffuse even when RT env visibility is off.
         * Colored specular closures are treated as metallic tint carriers and stay on HWRT. */
        const float NV = saturate(dot(cl.N, V));
        const float dielectric_grazing = pow(1.0f - NV, 5.0f);
        const float specular_tint = average(abs(cl.color));
        const bool dielectric_like_reflection = specular_tint < 0.35f;
        const bool metallic_like_reflection = specular_tint >= 0.35f;
        if (metallic_like_reflection) {
          indirect_light = float3(0.0f);
        }
        else if (dielectric_like_reflection && dielectric_grazing > 1.0e-4f) {
          float env_visibility_factor = 1.0f;
          if (use_environment_visibility) {
            env_visibility_factor = env_visibility.visibility;
          }
          LightProbeRay probe_ray = bxdf_lightprobe_ray(cl, P, V, thickness);
          float3 specular_probe = lightprobe_eval_with_direction(
              samp, cl, P, V, thickness, probe_ray.dominant_direction);
          indirect_light = specular_probe * env_visibility_factor * dielectric_grazing;
        }
        else {
          indirect_light = float3(0.0f);
        }
      }
      else if (use_environment_visibility) {
        if (specular_closure) {
          indirect_light = float3(0.0f);
        }
        else {
        const bool diffuse_world_closure = (cl.type == CLOSURE_BSDF_DIFFUSE_ID) ||
                                           (cl.type == CLOSURE_BSSRDF_BURLEY_ID);
        LightProbeRay probe_ray = bxdf_lightprobe_ray(cl, P, V, thickness);
        float3 world_direction = hardware_environment_visibility_direction(
            env_visibility, probe_ray.dominant_direction, cl.N);
        const bool valid_diffuse_world_direction = dot(safe_normalize(world_direction), cl.N) > 0.05f;
        if (!diffuse_world_closure || valid_diffuse_world_direction) {
          indirect_light = lightprobe_eval_with_direction(samp, cl, P, V, thickness, world_direction);
        }
        if (diffuse_world_closure) {
          /* Diffuse world lighting should stay strong when the point is open to the environment,
           * but enclosed interiors should lean on transported GI instead of keeping a broad direct
           * environment fill. Use a softer visibility gate than the specular path to avoid the
           * bright/dark halo artifacts that a raw multiplication produced earlier. */
          float diffuse_world_visibility = square(saturate((env_visibility.visibility - 0.05f) / 0.95f));
          indirect_light *= diffuse_world_visibility;
        }
        else {
          indirect_light *= env_visibility.visibility;
        }
        }
      }
