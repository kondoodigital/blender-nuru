...
void Sampling::init(const Scene *scene)
{
  ...
  viewport_sample_count_ = sample_count_;
}
...
void Sampling::request_extra_sample(uint64_t count)
{
  const uint64_t target_sample_count = sample_count_ + count;
  sample_count_ = max_uu(sample_count_, target_sample_count);
  viewport_sample_count_ = max_uu(viewport_sample_count_, target_sample_count);
}

void Sampling::request_viewport_extra_sample(uint64_t count)
{
  BLI_assert(inst_.is_viewport());
  viewport_sample_count_ = max_uu(viewport_sample_count_, sample_count_ + count);
}
