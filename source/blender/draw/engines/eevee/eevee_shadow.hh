...
class ShadowModule {
 ...
 private:
  ...
  /** Global switch. */
  bool enabled_ = true;
  /** Latched copy of `inst_.raytracing.use_hardware_shadows()` from last `init`. Toggling HWRT
   * shadows at runtime needs to force a light re-sync so the VSM tilemaps that were skipped
   * while HWRT was on get rebuilt on the way back, and vice versa. */
  bool hwrt_shadows_active_ = false;
