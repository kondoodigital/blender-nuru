...
class LightModule {
 ...
 private:
  ...
  /** Latched for the current sync when light-affecting state changed. */
  bool fast_gi_lighting_changed_ = false;
 ...
 public:
  ...
  int sun_lights_len() const
  {
    return sun_lights_len_;
  }

  int local_lights_len() const
  {
    return local_lights_len_;
  }

  bool fast_gi_lighting_changed() const
  {
    return fast_gi_lighting_changed_;
  }

  const LightCullingData &culling_data() const
  {
    return culling_data_buf_;
  }

  void append_sync_sun_lights(Vector<LightData> &r_sun_lights,
                              Vector<int> *r_world_sun_slots = nullptr) const;
  void append_sync_local_lights(Vector<LightData> &r_local_lights) const;
  ...
  template<typename PassType> void bind_no_cull_light_resources(PassType &pass)
  {
    pass.bind_ssbo("light_buf_no_cull", &light_buf_);
  }
