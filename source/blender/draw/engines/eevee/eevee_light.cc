...
{
  fast_gi_lighting_changed_ = false;
...
void LightModule::sync_light(const Object *ob, ObjectHandle &handle)
{
  ...
  if (light_object_recalc || light_data_recalc || !light.initialized) {
    ...
    light.sync(inst_.shadows,
               ...
               ob->light_linking,
               light_threshold_);
    if (!was_initialized || light_gpu_data_changed(previous_light, static_cast<const LightData &>(light)))
    {
      fast_gi_lighting_changed_ = true;
    }
  }
