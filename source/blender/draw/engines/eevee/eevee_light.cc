                 ...
                 float threshold)
{
  ...
  this->visible_in_reflections = (la->mode & LA_VISIBLE_REFLECTIONS) != 0;
  this->cast_shadow = (la->mode & LA_SHADOW) != 0;
...
static bool light_gpu_data_changed(const LightData &before, const LightData &after)
{
  return std::memcmp(&before, &after, sizeof(LightData)) != 0;
}
...
void LightModule::begin_sync()
{
  ...
  assign_if_different(use_sun_lights_, inst_.world.has_volume_absorption() == false);
  ...
  if (use_sun_lights_ && inst_.world.sky_sun_shadow_direction_get(sky_sun_direction)) {
...
void LightModule::sync_light(const Object *ob, ObjectHandle &handle)
{
  ...
  light.used = true;
  const bool light_object_recalc = (handle.recalc & (ID_RECALC_TRANSFORM | ID_RECALC_SHADING)) != 0;
  const bool light_data_recalc = (la.id.recalc & ID_RECALC_SHADING) != 0;
  const LightData previous_light = static_cast<const LightData &>(light);
  const bool was_initialized = light.initialized;
  if (light_object_recalc || light_data_recalc || !light.initialized) {
...
void LightModule::end_sync()
{
  ...
  /* Counters for the HWRT shadow visibility texture layout. The texture is filled CPU-side in
   * the natural per-type order of `append_sync_local_lights` / `append_sync_sun_lights` (both
   * iterate `light_map_.values()`), so the stable layer for each light is its position in that
   * iteration. Locals occupy layers `[0, local_lights_len_)`, suns occupy
   * `[local_lights_len_, local_lights_len_ + sun_lights_len_)`. We store the result on the
   * `LightData` itself so it survives the light culling Z-sort. */
  int local_shadow_layer_idx = 0;
  int sun_shadow_layer_idx = 0;
  ...
  for (auto it = light_map_.items().begin(); it != it_end; ++it) {
    ...
    if (is_sun_light(light.type)) {
      light.shadow_layer = local_lights_len_ + sun_shadow_layer_idx++;
    }
    else {
      light.shadow_layer = local_shadow_layer_idx++;
    }
  ...
  /* Nuru NIS: assign stable many-light cluster ids to local lights. Clusters partition the
   * locals by a Morton bucket over their positions quantized to the lights' bounding box,
   * giving a coarse stable spatial octree cell per light. The id rides in LightData through
   * the culling Z-sort (like `shadow_layer`), so per-tile kernels can aggregate candidate
   * importance per cluster and apply the learned cluster multipliers (stages N1/N2). */
  if (local_lights_len_ > 0) {
    float3 bounds_min = float3(FLT_MAX);
    float3 bounds_max = float3(-FLT_MAX);
    for (int i = sun_lights_len_; i < sun_lights_len_ + local_lights_len_; i++) {
      const float3 P = transform_location(light_buf_[i].object_to_world);
      bounds_min = math::min(bounds_min, P);
      bounds_max = math::max(bounds_max, P);
    }
    const float3 extent = math::max(bounds_max - bounds_min, float3(1.0e-3f));
    for (int i = sun_lights_len_; i < sun_lights_len_ + local_lights_len_; i++) {
      const float3 P = transform_location(light_buf_[i].object_to_world);
      const float3 ndc = (P - bounds_min) / extent;
      /* 2 bits per axis -> 64 octree cells, folded to HWRT_LIGHT_CLUSTER_COUNT buckets. */
      const uint qx = math::min(uint(ndc.x * 4.0f), 3u);
      const uint qy = math::min(uint(ndc.y * 4.0f), 3u);
      const uint qz = math::min(uint(ndc.z * 4.0f), 3u);
      const uint morton = (qx) | (qy << 2u) | (qz << 4u);
      light_buf_[i].cluster_id = int(morton % uint(HWRT_LIGHT_CLUSTER_COUNT));
    }
  }
...
void LightModule::append_sync_sun_lights(Vector<LightData> &r_sun_lights,
                                         Vector<int> *r_world_sun_slots) const
{
  r_sun_lights.reserve(r_sun_lights.size() + sun_lights_len_);
  if (r_world_sun_slots != nullptr) {
    r_world_sun_slots->reserve(r_world_sun_slots->size() + sun_lights_len_);
  }
  for (const auto item : light_map_.items()) {
    const ObjectKey &key = item.key;
    const Light &light = item.value;
    if (is_sun_light(light.type)) {
      LightData copy = light;
      int world_sun_slot = -1;
      if (key == world_sunlight_key_[WORLD_SUN_DIFFUSE]) {
        world_sun_slot = WORLD_SUN_DIFFUSE;
      }
      else if (key == world_sunlight_key_[WORLD_SUN_GLOSSY]) {
        world_sun_slot = WORLD_SUN_GLOSSY;
      }
      if (world_sun_slot != -1) {
        copy.color.x = -std::max(1.0e-6f, std::abs(copy.color.x));
      }
      r_sun_lights.append(copy);
      if (r_world_sun_slots != nullptr) {
        r_world_sun_slots->append(world_sun_slot);
      }
    }
  }
}

void LightModule::append_sync_local_lights(Vector<LightData> &r_local_lights) const
{
  r_local_lights.reserve(r_local_lights.size() + local_lights_len_);
  for (const Light &light : light_map_.values()) {
    if (!is_sun_light(light.type)) {
      r_local_lights.append(light);
    }
  }
}
...
void LightModule::culling_pass_sync()
{
  ...
  culling_ps_.init();
  {
    ...
    sub.bind_ssbo("out_world_sun_buf", &inst_.world.sunlight_rt);
    sub.bind_ssbo("out_world_sun_direction_buf", &inst_.world.sunlight_rt_direction);
