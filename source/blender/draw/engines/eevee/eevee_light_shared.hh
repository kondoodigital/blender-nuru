...
/** Nuru NIS: number of stable many-light clusters (network output width). */
#define HWRT_LIGHT_CLUSTER_COUNT 32
...
struct [[host_shared]] LightData {
  ...
  /* True if the light has shadow casting enabled by the user (LA_SHADOW).
   * Decouples "does this light cast shadow" from "does it have a VSM tilemap allocated".
   * HWRT shadow paths use this flag instead of `tilemap_index != LIGHT_NO_SHADOW` so that VSM
   * tilemaps can be skipped without disabling HWRT shadow evaluation. */
  bool32_t cast_shadow;
  /* Stable layer index in the HWRT shadow visibility texture.
   *
   * The visibility texture (`hardware_rt_shadow_visibility_tx`) is populated CPU-side by
   * `RayTraceModule::render_directional_shadow_visibility` in the *unsorted* `local_lights` /
   * `sun_lights` vector order, but the per-pixel lighting shaders iterate the *culled+sorted*
   * `light_buf`. The culling Z-sort changes whenever the view direction changes, which would
   * make a naive `texelFetch(..., l_idx)` read a different light's visibility from one rotation
   * to the next. We assign this layer once at sync time and copy it through culling so the
   * shaders can fetch the correct layer with `texelFetch(..., light.shadow_layer)`. */
  int shadow_layer;
  /* Nuru NIS: stable many-light cluster index in [0, HWRT_LIGHT_CLUSTER_COUNT). Assigned at
   * sync from a Morton-bucket spatial partition of the local lights; survives the culling
   * Z-sort like `shadow_layer`. Suns keep 0 (they are sampled separately). */
  int cluster_id;
  /* Nuru: LA_VISIBLE_REFLECTIONS - the light's image renders in sharp (mirror-like)
   * reflections. Off by default. */
  bool32_t visible_in_reflections;
  int _pad4;
