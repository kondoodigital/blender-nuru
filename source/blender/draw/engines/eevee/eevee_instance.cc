...
void Instance::draw_viewport()
{
  ...
  if (sampling.is_reset() && (sampling.sample_count() == 1) && raytracing.use_raytracing() &&
      raytracing.use_hardware_tracing_method())
  {
    uint64_t extra_samples = 0;
    if (raytracing.use_hardware_reflections() || raytracing.use_hardware_refractions()) {
      extra_samples = max_uu(extra_samples, 1);
    }
    if (raytracing.use_hardware_fast_gi()) {
      extra_samples = max_uu(extra_samples, 2);
    }
    if (extra_samples > 0) {
