...
void Instance::begin_sync()
{
  ...
  if (raytracing.use_hardware_tracing_method() && !VolumeModule::runtime_enabled()) {
    /* Nuru keeps the volume UI visible, but disables volume rendering on backends where the
     * volume runtime stays emergency-disabled (see `VolumeModule::runtime_enabled`). */
    use_volumes = false;
  }
...
static uint64_t nuru_extra_settle_samples_get(const Instance &inst)
{
  if (inst.raytracing.is_shader_preview_nuru_disabled() ||
      !inst.raytracing.use_raytracing() || !inst.raytracing.use_hardware_tracing_method())
  {
    return 0;
  }

  uint64_t extra_samples = 0;
  if (inst.raytracing.use_hardware_reflections() || inst.raytracing.use_hardware_refractions()) {
    extra_samples = max_uu(extra_samples, 1);
  }
...
void Instance::render_frame(RenderEngine *engine, RenderLayer *render_layer, const char *view_name)
{
  ...
  if (sampling.sample_count() == 1) {
    sampling.request_extra_sample(nuru_extra_settle_samples_get(*this));
  }
...
void Instance::draw_viewport()
{
  ...
  /* Deferred Hardware RT features need follow-up samples after a viewport reset before their
   * output becomes visible. Key this off the reset flag, not the low-level sample index:
   * interactive reprojection resets `viewport_sample_` but can keep the internal sampling
   * sequence (`sample_`) cycling, so a live edit may never look like `sample_index() == 0` even
   * though it is the first frame after a reset. Fast GI needs an extra settle sample beyond the
   * first visible reuse because its cascades are amortized in interactive viewports. */
  if (!raytracing.is_shader_preview_nuru_disabled() && sampling.is_reset() &&
      (sampling.sample_count() == 1) && raytracing.use_raytracing() &&
      raytracing.use_hardware_tracing_method())
  {
    const uint64_t extra_samples = nuru_extra_settle_samples_get(*this);
    if (extra_samples > 0) {
      sampling.request_viewport_extra_sample(extra_samples);
    }
  }
