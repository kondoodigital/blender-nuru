...
void ShadowModule::init()
{
  ...
  /* Nuru: HWRT shadow toggle must force a light re-sync so `shadow_ensure` rebuilds the VSM
   * shadow objects that were discarded while HWRT was active (and vice versa). Without this the
   * lights stay in the discarded state set by the previous `ShadowModule::end_sync` and the
   * VSM-eligible local lights end up with `tilemap_index == LIGHT_NO_SHADOW`.
   * Volumetrics keep VSM alive even with HWRT shadows on (the volume scatter pass samples only
   * the VSM atlas; see `end_sync`), so the tracked state includes the volume enable. At init
   * time this reads the previous frame's latched value, which delays the rebuild by one frame
   * when a volume appears or disappears - acceptable, the scene change restarts sampling
   * anyway. */
  const bool hwrt_vsm_skip = inst_.raytracing.use_hardware_shadows() && !inst_.volume.enabled();
  update_lights |= assign_if_different(hwrt_shadows_active_, hwrt_vsm_skip);
  if (update_lights) {
    /* Force light reset. */
...
void ShadowModule::end_sync()
{
   ...
   *
   * For every HWRT-covered light we discard any previously acquired shadow object and set
   * `tilemap_index = LIGHT_NO_SHADOW`. HWRT trace / fetch paths read `light.cast_shadow`
   * instead of `tilemap_index` so the per-light HWRT shadow still fires. The HWRT-shadow toggle
   * itself is tracked in `init` (`hwrt_shadows_active_`) so toggling forces a light re-sync that
   * rebuilds the VSM shadow object on the way back.
   *
   * Volumetrics exception: the volume scatter pass evaluates lights through the VSM atlas only
   * (froxels are not HWRT-shadow consumers), so while volumes are enabled this frame the VSM
   * tilemaps must stay allocated even with HWRT shadows on. Without this the fog is lit
   * unshadowed and renders as a uniform glow instead of carved shafts. `VolumeModule::end_sync`
   * runs before `ShadowModule::end_sync` (see `Instance::end_sync`), so the latched
   * `volume.enabled()` is valid here. Surfaces keep their HWRT shadows; the VSM cost is the
   * same one vanilla volumetrics always pay. */
  const bool hwrt_shadows_active = inst_.raytracing.use_hardware_shadows() &&
                                   !inst_.volume.enabled();
