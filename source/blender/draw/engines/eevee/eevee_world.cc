...
static bool world_has_linked_volume_output(const bNodeTree &ntree)
{
  const bNode *output_node = ntreeShaderOutputNode(const_cast<bNodeTree *>(&ntree), SHD_OUTPUT_EEVEE);
  if (output_node == nullptr) {
    return false;
  }

  const bNodeSocket *volume_socket = bke::node_find_socket(*output_node, SOCK_IN, "Volume");
  return volume_socket != nullptr && volume_socket->link != nullptr;
}
...
void World::sync_volume(const WorldHandle &world_handle, bool wait_ready)
{
  if (!VolumeModule::runtime_enabled()) {
    const bool had_volume = has_volume_;
    has_volume_ = has_volume_scatter_ = has_volume_absorption_ = false;
    inst_.pipelines.world_volume.sync(nullptr);
    if (had_volume) {
      inst_.volume.world_sync(world_handle);
    }
    UNUSED_VARS(wait_ready);
    return;
  }

  if (inst_.raytracing.use_hardware_tracing_method() && !VolumeModule::runtime_enabled()) {
    /* Nuru: on backends where the volume runtime stays emergency-disabled (see
     * `VolumeModule::runtime_enabled`), keep the world volume state zeroed under the hardware
     * tracing method. This also keeps the vanilla black-surface replacement for volume-absorption
     * worlds from triggering while no volume is actually rendered. Backends with the volume
     * runtime active sync the world volume normally. */
    const bool had_volume = has_volume_;
    has_volume_ = has_volume_scatter_ = has_volume_absorption_ = false;
    inst_.pipelines.world_volume.sync(nullptr);
    if (had_volume) {
  ...
  if (world && world->nodetree && world_has_linked_volume_output(*world->nodetree)) {
