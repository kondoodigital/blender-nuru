...
#define AS_SPAN(arr) Span<eShaderType>(arr, ARRAY_SIZE(arr))
  ...
  }
  {
    const eShaderType shader_list[] = {RAY_DENOISE_BILATERAL,
                                       ...
                                       RAY_GENERATE,
                                       RAY_HARDWARE_DIRECT_LIGHT_ACCUM,
                                       RAY_HARDWARE_DIRECT_LIGHT_DENOISE,
                                       ...
                                       RAY_HIT_EVAL_COUNT,
                                       RAY_HIT_EVAL_PREFIX,
                                       RAY_SHARED_INDIRECT_ACCUM,
                                       RAY_SHARED_INDIRECT_RECONSTRUCT,
                                       RAY_TILE_CLASSIFY,
                                       RAY_TILE_COMPACT,
                                       RAY_TRACE_FALLBACK,
                                       RAY_TRACE_HARDWARE_LIGHTING,
                                       RAY_TRACE_PLANAR,
                                       RAY_TRACE_SCENE_FINAL_SPECULAR_RESOLVE,
                                       RAY_TRACE_SCREEN};
...
const char *ShaderModule::static_shader_create_info_name_get(eShaderType shader_type)
{
  switch (shader_type) {
    ...
    case DEFERRED_PASS_EXPORT:
      return "eevee_deferred_pass_export";
    ...
    case RAY_HARDWARE_DIRECT_LIGHT_ACCUM:
      return "eevee_ray_hardware_direct_light_accum";
    case RAY_HARDWARE_DIRECT_LIGHT_DENOISE:
      return "eevee_ray_hardware_direct_light_denoise";
    case RAY_HARDWARE_DIRECT_LIGHT_TILE_COMPACT:
      return "eevee_ray_hardware_direct_light_tile_compact";
    case RAY_HARDWARE_DIRECT_LIGHT_VISIBILITY:
    ...
    case NIS_TRAIN:
      return "eevee_nuru_nis_train";
    case NIS_TRAIN_FEEDBACK:
      return "eevee_nuru_nis_train_feedback";
    case NIS_ADAM:
      return "eevee_nuru_nis_adam";
    case RAY_HARDWARE_TRACE_TILE_COMPACT:
      return "eevee_ray_hardware_trace_tile_compact";
    case RAY_HARDWARE_TILE_COMPACT:
      return "eevee_ray_hardware_tile_compact";
    case RAY_HIT_EVAL_COMPACT:
      return "eevee_ray_hit_eval_compact";
    case RAY_HIT_EVAL_COUNT:
      return "eevee_ray_hit_eval_count";
    case RAY_HIT_EVAL_PREFIX:
      return "eevee_ray_hit_eval_prefix";
    case RAY_SHARED_INDIRECT_ACCUM:
      return "eevee_ray_shared_indirect_accum";
    case RAY_SHARED_INDIRECT_RECONSTRUCT:
      return "eevee_ray_shared_indirect_reconstruct";
    case RAY_TRACE_FALLBACK:
      return "eevee_ray_trace_fallback";
    case RAY_TRACE_HARDWARE_LIGHTING:
      return "eevee_ray_trace_hardware_lighting";
    case RAY_TRACE_PLANAR:
      return "eevee_ray_trace_planar";
    case RAY_TRACE_SCENE_FINAL_SPECULAR_RESOLVE:
      return "eevee_ray_trace_scene_final_specular_resolve";
...
class StorageSlotAllocator {
  uint32_t available_slots_ = 0xFFFFu;
 ...
 public:
  void reserve_slots(const gpu::shader::ShaderCreateInfo &info)
  {
    ...
    using namespace blender::gpu::shader;
    auto reserve = [&](const ShaderCreateInfo::Resource &res) {
      if (res.bind_type == ShaderCreateInfo::Resource::BindType::STORAGE_BUFFER) {
        available_slots_ &= ~(uint32_t(1) << res.slot);
      }
    };
    for (const ShaderCreateInfo::Resource &res : info.pass_resources_) {
      reserve(res);
  ...
  int get_next_slot()
  {
    if ((available_slots_ & 0xFFFFu) == 0) {
      overflow_ = true;
      return -1;
    }
    return bitscan_forward_clear_uint(&available_slots_);
  }
                                              ...
                                              const bool use_shader_to_rgba)
{
    ...
    default:
      switch (pipeline_type) {
        ...
        case MAT_PIPE_CAPTURE:
          ...
          info.name_ += "_capture";
          break;
        case MAT_PIPE_HIT_EVAL:
          pipeline_info_name = "eevee_surf_hit_eval";
          info.name_ += "_hit_eval";
          break;
  ...
  switch (geometry_type) {
    ...
    case MAT_GEOM_CURVES:
      ...
      break;
    case MAT_GEOM_MESH:
      geometry_info_name = (pipeline_type == MAT_PIPE_HIT_EVAL) ? "eevee_geom_hit_mesh" :
                                                                 "eevee_geom_mesh";
      info.name_ += (pipeline_type == MAT_PIPE_HIT_EVAL) ? "_hit_mesh" : "_mesh";
      break;
...
void ShaderModule::material_create_info_amend(GPUMaterial *gpumat, GPUCodegenOutput *codegen_)
{
  ...
  gpu::shader::GPUCodegenCreateInfo &codegen_info = *reinterpret_cast<
      gpu::shader::GPUCodegenCreateInfo *>(codegen.create_info);
  const bool is_hit_eval_mesh = (geometry_type == MAT_GEOM_MESH) &&
                                (pipeline_type == MAT_PIPE_HIT_EVAL);
  Map<std::string, int> hit_eval_attr_slots;
  Map<std::string, std::string> hit_eval_attr_symbols;
  ...
  StorageSlotAllocator storage_slots;
  storage_slots.reserve_slots(info);
  ...
  if (GPU_material_flag_get(gpumat, GPU_MATFLAG_THIN_GLASS)) {
    info.define("MAT_THIN_GLASS");
  }
  ...
  if (GPU_material_flag_get(gpumat, GPU_MATFLAG_REFLECTION_MAYBE_COLORED) == false &&
      GPU_material_flag_get(gpumat, GPU_MATFLAG_THIN_GLASS) == false)
  {
  ...
  if (is_hit_eval_mesh && (closure_bits & CLOSURE_REFLECTION)) {
    /* Sparse mirror replay may disable a dielectric Principled reflection lobe after material
     * evaluation. Keep a separate base bin available so the replayed diffuse texture cannot be
     * lost if the material flags under-report the modified Principled diffuse component. */
    closure_bin_count = max_ii(closure_bin_count, 2);
  }
  if (is_hit_eval_mesh && (closure_bits & CLOSURE_DIFFUSE) && (closure_bits & CLOSURE_REFLECTION) &&
      (closure_bits & CLOSURE_REFRACTION))
  {
    /* Scene-final Principled replay needs diffuse, reflection, and refraction available
     * simultaneously. The normal deferred bin budget packs diffuse and refraction together, which
     * makes hit-eval flip between them as their weights cross. */
    closure_bin_count = max_ii(closure_bin_count, 3);
  }
  ...
  if (pipeline_type == MAT_PIPE_DEFERRED) {
    ...
    if (closure_bin_count == 2) {
      ...
      const bool colorless_reflection =
          !GPU_material_flag_get(gpumat, GPU_MATFLAG_REFLECTION_MAYBE_COLORED) &&
          !GPU_material_flag_get(gpumat, GPU_MATFLAG_THIN_GLASS);
  ...
  switch (geometry_type) {
    case MAT_GEOM_MESH:
      ...
      else if (is_hit_eval_mesh) {
        for (const auto &input : info.vertex_inputs_) {
          if (input.name == "orco") {
            /* Sparse hit-eval can always reconstruct Generated/Object coordinates from the traced
             * hit position. Keep ORCO off the required SSBO set here so reflection replay does not
             * fail closed on evaluated meshes that have no dedicated ORCO buffer. */
            continue;
          }
          const int attr_binding_slot = storage_slots.get_next_slot();
          if (attr_binding_slot == -1) {
            break;
          }

          const std::string attr_symbol = (input.name == "a") ? "eevee_default_uv_attr" : input.name;
          char attr_buffer_name[32];
          SNPRINTF(attr_buffer_name, "%s[]", attr_symbol.c_str());
          info.storage_buf(attr_binding_slot,
                           Qualifier::read,
                           "uint",
                           codegen_info.name_buffer.append_sampler_name(attr_buffer_name),
                           Frequency::GEOMETRY);

          char gpu_attr_name[32];
          SNPRINTF(gpu_attr_name, "gpu_attr_%d", attr_binding_slot);
          info.push_constant(gpu::shader::Type::int2_t,
                             codegen_info.name_buffer.append_sampler_name(gpu_attr_name));

          char gpu_attr_meta_name[32];
          SNPRINTF(gpu_attr_meta_name, "gpu_attr_%d_meta", attr_binding_slot);
          info.push_constant(gpu::shader::Type::int_t,
                             codegen_info.name_buffer.append_sampler_name(gpu_attr_meta_name));

          hit_eval_attr_slots.add_new(std::string(input.name), attr_binding_slot);
          hit_eval_attr_symbols.add_new(std::string(input.name), attr_symbol);
        }
        info.vertex_inputs_.clear();
      }
      break;
  ...
  auto hit_eval_loader_name = [](const GPUMaterialAttribute &attr) -> const char * {
    if (attr.is_hair_length || attr.is_hair_intercept) {
      return "hit_attr_load_float";
    }
    if (attr.type == CD_ORCO) {
      return "hit_attr_load_orco_hit_eval";
    }
    if (attr.type == CD_TANGENT) {
      return "hit_attr_load_tangent";
    }
    return "hit_attr_load_float4";
  };

  std::stringstream attr_load;
  attr_load << "{\n";
  if (is_hit_eval_mesh) {
    for (GPUMaterialAttribute &attr : GPU_material_attributes(gpumat)) {
      const int *attr_slot = hit_eval_attr_slots.lookup_ptr(std::string(attr.input_name));
      if ((attr.type == CD_ORCO) && (attr_slot == nullptr)) {
        /* Sparse hit-eval can load the real ORCO buffer when the material requested one. Only
         * synthesize Generated/Object fallback when the evaluated mesh truly has no ORCO layer, so
         * deformed meshes keep their authored ORCO semantics instead of collapsing to grey
         * generated coordinates. */
        attr_load << "  var_attrs.v" << attr.id << " = eevee_hit_eval_generated_orco();\n";
        continue;
      }
      ...
      char gpu_attr_name[32];
      SNPRINTF(gpu_attr_name, "gpu_attr_%d", *attr_slot);
      char gpu_attr_meta_name[32];
      SNPRINTF(gpu_attr_meta_name, "gpu_attr_%d_meta", *attr_slot);
      const std::string *attr_symbol = hit_eval_attr_symbols.lookup_ptr(std::string(attr.input_name));
      const StringRefNull attr_symbol_name = (attr_symbol != nullptr) ? attr_symbol->c_str() :
                                                                     attr.input_name;
      attr_load << "  var_attrs.v" << attr.id << " = " << hit_eval_loader_name(attr) << "(domain, "
                << attr_symbol_name << ", " << gpu_attr_name << ", " << gpu_attr_meta_name << ", "
                << attr.id << ");\n";
    }
  }
  else {
  ...
  const bool need_hit_eval_vertex_attr_load = is_hit_eval_mesh;
  if (do_vertex_attrib_load || need_hit_eval_vertex_attr_load) {
    /* Hit-eval still replays `nodetree_displacement()` in the vertex shader. Production materials
     * can reference Generated/Attribute/TexCoord inputs there, so the sparse hit-eval path needs
     * the same generated `var_attrs` globals and attribute loader on the vertex side too instead of
     * only in the fragment stage. */
  ...
  if (pipeline_type != MAT_PIPE_VOLUME_OCCUPANCY) {
    ...
    /* Float literals: the value is multiplied with `nodetree_thickness()` and plain GLSL has no
     * implicit bool-to-float conversion. 0.0/1.0 matches MSL's `false`/`true` promotion. */
    info.define("thickness_mode", thickness_type == MAT_THICKNESS_SLAB ? "0.0f" : "1.0f");
...
struct CallbackThunk {
  ...
  bool force_dithered_refraction_with_nuru;
};

static bool material_uses_raytraced_transmission(const blender::Material *blender_mat,
                                                 const bool force_dithered_refraction_with_nuru)
{
  ...
  return force_dithered_refraction_with_nuru;
}
...
static GPUPass *pass_replacement_cb(void *void_thunk, GPUMaterial *mat)
{
  ...
  bool has_raytraced_transmission = material_uses_raytraced_transmission(
      blender_mat, thunk->force_dithered_refraction_with_nuru);
                                               ...
                                               blender::Material *default_mat)
{
  ...
  const bool force_dithered_refraction_with_nuru =
      draw_ctx != nullptr && draw_ctx->scene != nullptr &&
      draw_ctx->v3d != nullptr && draw_ctx->v3d->shading.type == OB_RENDER &&
      (draw_ctx->scene->eevee.flag & SCE_EEVEE_SSR_ENABLED) != 0 &&
      draw_ctx->scene->eevee.ray_tracing_method == RAYTRACE_EEVEE_METHOD_HARDWARE;

  CallbackThunk thunk = {this, default_mat, force_dithered_refraction_with_nuru};
