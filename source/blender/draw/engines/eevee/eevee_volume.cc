...
void VolumeModule::disable_runtime()
{
  enabled_ = false;
  ...
  data_.tile_size = 1;
  data_.tile_size_lod = 0;
  data_.coord_scale = float2(1.0f);
  data_.main_view_extent = float2(1.0f);
  data_.main_view_extent_inv = float2(1.0f);
  data_.tex_size = int3(1);
  data_.inv_tex_size = float3(1.0f);
  data_.shadow_steps = 0;
  data_.light_clamp = 1e20f;
  data_.history_opacity = 0.0f;

  occupancy_tx_.free();
  hit_count_tx_.free();
  hit_depth_tx_.free();
  front_depth_tx_.free();
...
{
  if (!runtime_enabled()) {
    /* Nuru disables all runtime volumetrics. This preserves the volume safety invariants by
     * preventing froxel texture allocation, volume material passes, and GPU volume stores. */
    disable_runtime();
    return;
  }
...
bool VolumeModule::will_enable() const
{
  ...
  /* Nuru: with the Vulkan volume runtime active (see `runtime_enabled`), the hardware tracing
   * method no longer disables volume pass submission. The vanilla pass set runs unchanged with
   * the invariant-Q ordering in `eevee_view.cc` and the invariant-A integration bounds clamp. */
...
void VolumeModule::end_sync()
{
  ...
  /* Vanilla behavior: latch the enable state computed from this frame's sync. */
