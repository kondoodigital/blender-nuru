...
struct [[host_shared]] HardwareDirectLightVisibilitySample {
  ...
  /** RT visibility for the selected local-light sample. Nuru: RGB so refractive shadows
   * (e.g. glass) can carry per-channel attenuation through to the receiver. `packed_float3`
   * keeps 3+1 packing with the trailing scalar so the struct stays 16-byte aligned. */
  packed_float3 local_visibility;
  ...
  packed_float3 sun_visibility;
  /** Importance used when selecting the sun-light sample. */
  float sun_importance;
  /** Nuru NIS training feedback: luminance of the weighted local-light contribution
   * (realized L/p estimate), written back by the accumulation kernel. */
  float train_luma;
  float _pad0;
  float _pad1;
  float _pad2;
...
struct [[host_shared]] RayTraceData {
  ...
  /** Nuru NIS: enables the neural cluster-multiplier network in the direct-light kernels. */
  bool32_t hardware_nis_enable;
  /** Reserved (was the removed fast-GI field validity). Keep for UBO layout. */
  bool32_t _hardware_reserved1;
  ...
  /** Reserved (was the removed fast-GI cascade grid resolution). Keep for UBO layout. */
  int _hardware_reserved2;
  /** Reserved (was the removed fast-GI cascade count). Keep for UBO layout. */
  int _hardware_reserved3;
  ...
  /** Reserved (was the removed fast-GI freeze debug switch). Keep for UBO layout. */
  ...
  /** Enables the per-pixel Secondary GI receiver kernels (diffuse receivers seen through
   * scene-final Hardware reflections/refractions). */
  bool32_t use_hardware_fast_gi_secondary;
  /** Nuru: this padding is layout-critical. The scalar run above ends at a 12-byte offset; under
   * std140 the `float4` array below rounds up to the next 16-byte boundary while the C++ side
   * packs it directly, shifting every later member of `UniformData` (volumes, pipeline, ...) on
   * the GPU. That mismatch fed garbage `volumes.tex_size` to the volume integration loop
   * (unbounded froxel stores, NVIDIA Xid 109 wedge) and NaN depth mapping (black volumetrics).
   * Keep `_hardware_reserved4` 16-byte aligned on both sides. */
  int _pad1;
  /** Reserved (was the removed fast-GI cascade configs). Keep for UBO layout. */
  float4 _hardware_reserved4[3];
...
#ifndef GPU_SHADER
/** Maximum specular reflection/refraction events for Full RT (host-only; matches GPU_HARDWARE_RAYTRACE_SPECULAR_MAX_BOUNCES). */
static constexpr int HWRT_SPECULAR_MAX_BOUNCES = 8;
}  // namespace blender::eevee
#endif
