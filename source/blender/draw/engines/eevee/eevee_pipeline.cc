                              ...
                              const float background_blur)
{
  ...
  world_ps_.material_set(manager, gpumat, false);
...
void WorldPipeline::sync(GPUMaterial *gpumat)
{
  ...
  Manager &manager = *inst_.manager;
  pass.material_set(manager, gpumat, false);
...
void WorldVolumePipeline::sync(GPUMaterial *gpumat)
{
  ...
  world_ps_.material_set(*inst_.manager, gpumat, false);
                                          ...
                                          bool write_depth)
{
  ...
  DRWState state = DRW_STATE_WRITE_COLOR | depth_state |
                   (write_depth ? DRW_STATE_WRITE_DEPTH : DRWState(0)) |
...
void DeferredLayer::begin_sync(DRWState gbuffer_depth_state, bool gbuffer_write_depth)
{
  {
    prepass_ps_.init();
    /* Textures. */
  ...
  this->gbuffer_pass_sync(inst_, gbuffer_depth_state, gbuffer_write_depth);
}

bool DeferredLayer::do_merge_direct_indirect_eval(const Instance &inst)
{
  if (inst.raytracing.use_hardware_tracing_method()) {
    /* Nuru: GI is feature-owned. If Hardware GI is off, diffuse GI is off; do not fall back to
     * classic Eevee SSGI/probe GI while the Nuru method is selected. */
    return false;
  }
                             ...
                             bool next_layer_has_transmission)
{
  ...
  use_hardware_rt_shadows_ = inst_.raytracing.use_hardware_shadows();
  use_hardware_rt_environment_visibility_ = inst_.raytracing.use_hardware_environment();
  ...
  defer_scene_final_hardware_specular_ =
      use_raytracing_ && inst_.raytracing.use_hardware_tracing_method() &&
      (((closure_bits_ & CLOSURE_REFLECTION) && inst_.raytracing.use_hardware_reflections()) ||
       ((closure_bits_ & CLOSURE_REFRACTION) && inst_.raytracing.use_hardware_refractions()));
...

  {
...

    {
      ...
      }
      {
        ...
        const bool use_hardware_direct_light = inst_.raytracing.use_hardware_tracing_method() &&
                                               use_hardware_rt_shadows_ && inst_.is_viewport();
        ...
        auto set_specialization_constants =
            [&](PassSimple::Sub &sub, gpu::Shader *sh, bool use_transmission) {
              ...
              sub.specialize_constant(sh, "use_hardware_rt_shadows", &use_hardware_rt_shadows_);
              sub.specialize_constant(sh,
                                      "use_hardware_rt_environment_visibility",
                                      &use_hardware_rt_environment_visibility_);
              /* By value: `use_hardware_direct_light` is a sync-time local. Passing its address
               * makes the pass read a dead stack slot at submit time (stack-use-after-return). */
              sub.specialize_constant(sh, "use_hardware_direct_light", use_hardware_direct_light);
              sub.specialize_constant(
                  sh, "defer_hardware_specular_indirect", &defer_scene_final_hardware_specular_);
        ...
        for (int i = min_ii(3, closure_count_) - 1; i >= 0; i--) {
          ...
          sub.bind_texture("hardware_rt_shadow_visibility_tx",
                           inst_.raytracing.directional_shadow_visibility_tx());
          sub.bind_texture("hardware_rt_environment_visibility_tx",
                           inst_.raytracing.environment_visibility_tx());
          sub.bind_resources(inst_.uniform_data);
    ...
    }
    {
      ...
      pass.specialize_constant(sh, "use_shared_indirect", &indirect_result_.use_shared_indirect);
      ...
      pass.bind_texture("shared_indirect_tx", &shared_indirect_tx_);
      pass.bind_texture("hardware_rt_caustics_tx", inst_.raytracing.caustics_tx());
      pass.bind_texture("hardware_direct_light_tx", inst_.raytracing.direct_light_accum_tx());
    ...
    /* Post-combine pass-export compute. See `pass_export_ps_` doc comment in the header for
     * the rationale; the dispatch is skipped entirely when no per-pass slot is requested. */
    any_pass_export_enabled_ = (rbuf_data.diffuse_color_id != -1) ||
                               (rbuf_data.diffuse_light_id != -1) ||
                               (rbuf_data.specular_color_id != -1) ||
                               (rbuf_data.specular_light_id != -1) ||
                               (rbuf_data.normal_id != -1) ||
                               (rbuf_data.position_id != -1);
    if (any_pass_export_enabled_) {
      PassSimple &pass = pass_export_ps_;
      pass.init();
      gpu::Shader *sh = inst_.shaders.static_shader_get(DEFERRED_PASS_EXPORT);
      pass.specialize_constant(sh, "use_split_radiance", use_split_radiance_);
      pass.specialize_constant(sh, "use_shared_indirect", &indirect_result_.use_shared_indirect);
      pass.specialize_constant(
          sh, "defer_hardware_specular_indirect", defer_scene_final_hardware_specular_);
      pass.specialize_constant(sh, "pass_diffuse_color_id", rbuf_data.diffuse_color_id);
      pass.specialize_constant(sh, "pass_diffuse_light_id", rbuf_data.diffuse_light_id);
      pass.specialize_constant(sh, "pass_specular_color_id", rbuf_data.specular_color_id);
      pass.specialize_constant(sh, "pass_specular_light_id", rbuf_data.specular_light_id);
      pass.specialize_constant(sh, "pass_normal_id", rbuf_data.normal_id);
      pass.specialize_constant(sh, "pass_position_id", rbuf_data.position_id);
      pass.shader_set(sh);
      pass.bind_texture("direct_radiance_1_tx", &direct_radiance_txs_[0]);
      pass.bind_texture("direct_radiance_2_tx", &direct_radiance_txs_[1]);
      pass.bind_texture("direct_radiance_3_tx", &direct_radiance_txs_[2]);
      ...
      pass.bind_resources(inst_.hiz_buffer.front);
      pass.barrier(GPU_BARRIER_TEXTURE_FETCH | GPU_BARRIER_SHADER_IMAGE_ACCESS);
      /* Dispatch size depends on the runtime extent which is only known in `render()`. Record by
       * pointer: appending the dispatch in `render()` would add one command per sample on image
       * renders since the pass is only (re)recorded during sync. */
      pass.dispatch(&pass_export_dispatch_size_);
    }
  }
}
                                    ...
                                    gpu::Texture *radiance_behind_tx)
{
  ...
  gpu::Texture *visibility_depth_tx = static_cast<gpu::Texture *>(rb.depth_tx);
  gpu::Texture *visibility_normal_tx = static_cast<gpu::Texture *>(inst_.gbuffer.normal_tx);
  inst_.raytracing.render_environment_visibility(
      render_view, visibility_depth_tx, visibility_normal_tx, extent);

  gpu::Texture *shadow_depth_tx = static_cast<gpu::Texture *>(rb.depth_tx);
  gpu::Texture *shadow_normal_tx = static_cast<gpu::Texture *>(inst_.gbuffer.normal_tx);
  ...
  if (use_raytracing_) {
    if (defer_scene_final_hardware_specular_) {
      ...
      if ((closure_bits_ & (CLOSURE_DIFFUSE | CLOSURE_SSS)) && inst_.raytracing.use_hardware_rt_gi()) {
        pre_feature_mask |= RAYTRACE_EEVEE_HARDWARE_GI;
      }
      indirect_result_ = inst_.raytracing.render_phase(rt_buffer,
                                                       nullptr,
                                                       radiance_behind_tx,
                                                       closure_bits_,
                                                       main_view,
  ...
  shared_indirect_tx_ = indirect_result_.use_shared_indirect ?
                            static_cast<gpu::Texture *>(indirect_result_.shared_indirect) :
                            static_cast<gpu::Texture *>(dummy_black);
  ...
  if (any_pass_export_enabled_) {
    /* Post-combine pass-export: writes per-pass values into `rp_color_tx` from compute
     * because the combine fragment shader writes do not reach under Nuru on Metal. */
    pass_export_dispatch_size_ = int3(math::divide_ceil(extent, int2(8)), 1);
    inst_.manager->submit(pass_export_ps_, render_view);
  }
...
void DeferredPipeline::begin_sync()
{
  ...
  thin_glass_layer_.begin_sync(DRW_STATE_DEPTH_GREATER, true);
}
...
void DeferredPipeline::end_sync()
{
  ...
  const bool has_refraction_layer = !refraction_layer_.is_empty();

  opaque_layer_.end_sync(true, !has_refraction_layer, refraction_layer_.has_transmission());
...
static bool material_uses_raytraced_transmission(const Instance &inst,
                                                 ...
                                                 GPUMaterial *gpumat)
{
  ...
  return inst.raytracing.use_raytracing() && inst.raytracing.use_hardware_tracing_method();
}

static bool material_uses_thin_glass_pass_through(GPUMaterial *gpumat)
{
  return GPU_material_flag_get(gpumat, GPU_MATFLAG_THIN_GLASS);
}
...
PassMain::Sub *DeferredPipeline::prepass_add(blender::Material *blender_mat,
                                             ...
                                             bool has_motion)
{
  if (material_uses_thin_glass_pass_through(gpumat)) {
    UNUSED_VARS(blender_mat, has_motion);
    return nullptr;
  }
  if (material_uses_raytraced_transmission(opaque_layer_.inst_, blender_mat, gpumat)) {
...
{
  if (material_uses_thin_glass_pass_through(gpumat)) {
    return thin_glass_layer_.material_add(blender_mat, gpumat);
  }
  if (material_uses_raytraced_transmission(opaque_layer_.inst_, blender_mat, gpumat)) {
...
void DeferredPipeline::render_thin_glass_overlay(View &render_view, int2 extent)
{
  if (thin_glass_layer_.is_empty()) {
    return;
  }

  GPU_debug_group_begin("Deferred.ThinGlassOverlay");
  thin_glass_overlay_radiance_tx_.acquire(extent, RenderBuffers::color_format);
  GBuffer &gbuf = thin_glass_layer_.inst_.gbuffer;
  RenderBuffers &rbufs = thin_glass_layer_.inst_.render_buffers;
  thin_glass_overlay_fb_.ensure(
      GPU_ATTACHMENT_TEXTURE(rbufs.depth_tx),
      GPU_ATTACHMENT_TEXTURE(thin_glass_overlay_radiance_tx_),
  ...
  thin_glass_layer_.radiance_behind_tx_ = thin_glass_layer_.dummy_black;
  GPU_framebuffer_bind(thin_glass_overlay_fb_);
  thin_glass_layer_.inst_.manager->submit(thin_glass_layer_.gbuffer_ps_, render_view);
  thin_glass_overlay_radiance_tx_.release();
  GPU_debug_group_end();
}
