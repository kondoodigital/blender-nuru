...
bool DeferredLayer::do_merge_direct_indirect_eval(const Instance &inst)
{
  if (inst.raytracing.use_hardware_tracing_method()) {
    if (inst.raytracing.has_hardware_fast_gi_field() || inst.raytracing.use_hardware_rt_gi()) {
      return false;
    }
    /* With Hardware RT selected but GI disabled, keep the classic deferred lightprobe path alive
     * only for the separate Environment feature so diffuse environment contribution falls back to
     * Eevee's baseline instead of blacking out the surface. */
    return inst.raytracing.use_hardware_environment();
  }
  return !inst.raytracing.use_raytracing() ||
         (inst.raytracing.use_hardware_fast_gi() &&
          (!inst.raytracing.use_hardware_rt_gi() || inst.raytracing.use_hardware_gi_refine()));
}
                             ...
                             bool next_layer_has_transmission)
{
...

  {
    ...
    }
    {
      ...
      pass.bind_texture("hardware_fast_gi_tx",
                        inst_.raytracing.fast_gi_tx(),
                        fast_gi_field_sampler);
      pass.bind_texture("hardware_fast_gi_visibility_tx",
                        inst_.raytracing.fast_gi_visibility_tx(),
                        fast_gi_field_sampler);
                                    ...
                                    gpu::Texture *radiance_behind_tx)
{
  ...
  const bool indirect_gi_cache_rendering = inst_.raytracing.is_hardware_indirect_gi_cache_rendering();
  const bool previous_use_hardware_rt_shadows = use_hardware_rt_shadows_;
  const bool previous_use_hardware_rt_environment_visibility = use_hardware_rt_environment_visibility_;
  if (indirect_gi_cache_rendering) {
    use_hardware_rt_shadows_ = false;
    use_hardware_rt_environment_visibility_ = false;
  }
  gpu::Texture *visibility_depth_tx = indirect_gi_cache_rendering ?
                                          nullptr :
                                          static_cast<gpu::Texture *>(rb.depth_tx);
  gpu::Texture *visibility_normal_tx = indirect_gi_cache_rendering ?
                                           nullptr :
  ...
  if (use_raytracing_) {
    if (defer_scene_final_hardware_specular_) {
      ...
      if (!indirect_gi_cache_rendering && inst_.raytracing.use_hardware_caustics()) {
        if ((closure_bits_ & CLOSURE_REFLECTION) && inst_.raytracing.use_hardware_reflections()) {
          pre_feature_mask |= RAYTRACE_EEVEE_HARDWARE_REFLECTIONS;
        }
        if ((closure_bits_ & CLOSURE_REFRACTION) && inst_.raytracing.use_hardware_refractions()) {
          pre_feature_mask |= RAYTRACE_EEVEE_HARDWARE_REFRACTIONS;
        }
      }
      if (!indirect_gi_cache_rendering && (pre_feature_mask != 0) &&
          inst_.raytracing.use_hardware_environment())
      {
        pre_feature_mask |= RAYTRACE_EEVEE_HARDWARE_ENVIRONMENT;
      }
      const eClosureBits active_closures = indirect_gi_cache_rendering ?
                                               (closure_bits_ & (CLOSURE_DIFFUSE | CLOSURE_SSS)) :
                                               closure_bits_;
      ...
      if (indirect_gi_cache_rendering) {
        const uint32_t pre_feature_mask = ((closure_bits_ & (CLOSURE_DIFFUSE | CLOSURE_SSS)) &&
                                           inst_.raytracing.use_hardware_rt_gi()) ?
  ...
  radiance_feedback_tx_ = rt_buffer.feedback_ensure(
      !use_feedback_output_ && !inst_.raytracing.use_hardware_fast_gi(), extent);
  ...
  if (inst_.raytracing.use_hardware_fast_gi()) {
    inst_.raytracing.update_hardware_fast_gi_field(render_view, rb.depth_tx, rb.combined_tx, extent);
  }
  ...
  if (indirect_gi_cache_rendering) {
    use_hardware_rt_shadows_ = previous_use_hardware_rt_shadows;
    use_hardware_rt_environment_visibility_ = previous_use_hardware_rt_environment_visibility;
  }
...
PassMain::Sub *DeferredPipeline::prepass_add(blender::Material *blender_mat,
                                             ...
                                             bool has_motion)
{
  if (material_uses_raytraced_transmission(opaque_layer_.inst_, blender_mat)) {
