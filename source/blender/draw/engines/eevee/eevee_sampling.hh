...
class Sampling {
 private:
  ...
  /** Viewport-only finish budget. Can exceed `sample_count_` for post-reset settle frames. */
  uint64_t viewport_sample_count_ = 64;
 ...
 public:
  ...
  /* Request additional render samples beyond the nominal TAA budget. */
  void request_extra_sample(uint64_t count = 1);
  /* Viewport Only: request additional render samples beyond the nominal TAA budget. */
  void request_viewport_extra_sample(uint64_t count = 1);
  ...
  {
    return (viewport_sample_ >= viewport_sample_count_) && !interactive_mode_;
  }
  ...
  uint64_t viewport_sample_count() const
  {
    return viewport_sample_count_;
  }

  uint64_t viewport_sample_index() const
  {
    return (viewport_sample_ > 0) ? uint64_t(viewport_sample_) : 0;
  }
