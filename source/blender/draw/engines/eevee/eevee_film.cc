...
void Film::end_sync()
{
  ...
  if (inst_.is_viewport() && inst_.sampling.is_reset()) {
    /* A viewport reset means scene data changed. Even if Eevee keeps interactive reprojection
     * enabled for the next few frames, reusing the previous film history would blend pre-edit
     * pixels back over the updated result. */
