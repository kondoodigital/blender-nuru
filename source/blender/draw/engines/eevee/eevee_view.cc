...
void ShadingView::render()
{
  ...
  inst_.pipelines.deferred.render_thin_glass_overlay(render_view_, extent_);

  GPU_memory_barrier(GPU_BARRIER_FRAMEBUFFER | GPU_BARRIER_TEXTURE_FETCH);
  scene_final_radiance_tx_.acquire(extent_, RenderBuffers::color_format);
  GPU_texture_copy(scene_final_radiance_tx_, rbufs.combined_tx);
  inst_.raytracing.render_scene_final_specular(rt_buffer_scene_final_specular_,
                                               scene_final_radiance_tx_,
                                               inst_.pipelines.deferred.closure_bits_get(),
                                               main_view_,
                                               render_view_);
  scene_final_radiance_tx_.release();
