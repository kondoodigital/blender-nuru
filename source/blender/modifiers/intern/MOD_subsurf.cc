...
static Mesh *modify_mesh(ModifierData *md, const ModifierEvalContext *ctx, Mesh *mesh)
{
  ...
  if ((ctx->flag & MOD_APPLY_TO_ORIGINAL) == 0) {
    ...
    if (md == reinterpret_cast<const ModifierData *>(smd)) {
      bool has_gpu_subdiv = BKE_subsurf_modifier_can_do_gpu_subdiv(smd, mesh);
      if (scene->eevee.ray_tracing_method == RAYTRACE_EEVEE_METHOD_HARDWARE) {
        /* Nuru Hardware RT currently needs Subsurf to stay on the CPU-evaluated path.
         * Keep this override scoped to Nuru so classic Eevee plus Subsurf behavior is unchanged. */
        has_gpu_subdiv = false;
      }
